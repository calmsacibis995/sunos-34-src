#ifndef lint
static	char sccsid[] = "@(#)if_il.c 1.1 86/09/25 SMI"; /* from UCB 6.2 83/09/24 */
#endif
...
#define	ILWATCHINTERVAL	60		/* once every 60 seconds */
	...
	struct ether_addr is_hwaddr;	/* Hardware Ethernet address */
} il_softc[NIL];
...
justarp:
	/* set physical addr */
	s = splimp();
	if (bcmp((caddr_t)&is->is_addr, (caddr_t)&is->is_hwaddr,
	    sizeof (struct ether_addr)) == 0) {		/* Hardware address */
		addr->il_csr = ILC_DEFPA;
		while ((addr->il_csr&IL_CDONE) == 0)
			;
	} else {					/* Soft address */
		int ubaddr;

		ubaddr = uballoc(ui->ui_ubanum, (caddr_t)&is->is_addr,
		    sizeof (struct ether_addr), 0);
		addr->il_bar = ubaddr & 0xffff;
		addr->il_bcr = sizeof (struct ether_addr);
		addr->il_csr = ((ubaddr >> 2) & IL_EUA)|ILC_LDPA;
		while ((addr->il_csr&IL_CDONE) == 0)
			;
		if (addr->il_csr&IL_STATUS)
			printf("il%d: set addr failed, csr=%b\n", ui->ui_unit,
				addr->il_csr, IL_BITS);
...
ilioctl(ifp, cmd, data)
	...
	caddr_t data;
{
	struct il_softc *is = &il_softc[ifp->if_unit];
	struct sockaddr *sa;
	struct sockaddr_in *sin;
	...
	case SIOCSIFADDR:
		...
		if (sa->sa_family == AF_UNSPEC) {
			if (sa->sa_data[0] & 1)	/* broad or multi-cast */
				return (EINVAL);
			is->is_addr = *(struct ether_addr *)sa->sa_data;
			ilinit(ifp->if_unit);
			break;
		}
