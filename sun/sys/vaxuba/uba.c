/*      @(#)uba.c 1.1 86/09/25 SMI; from UCB 6.1 83/08/21       */
	...
	struct buf *bp;
{
	...
	while ((reg = rmalloc(uh->uh_map, (long)npf)) == 0) {
		...
		uh->uh_mrwant++;
		(void) sleep((caddr_t)&uh->uh_mrwant, PSWP);
	}
	bdp = 0;
	if (flags & UBA_NEEDBDP) {
		while ((bdp = ffs(uh->uh_bdpfree)) == 0) {
			...
			uh->uh_bdpwant++;
			(void) sleep((caddr_t)&uh->uh_bdpwant, PSWP);
		}
	...
	caddr_t vaddress;
{
	...
	Sysmap[btop(((u_int)(vaddress)) - KERNELBASE)].pg_prot = (PG_UW>>27);
}
	...
	caddr_t vaddress;
{
	...
	Sysmap[btop(((u_int)(vaddress)) - KERNELBASE)].pg_prot = (PG_KW>>27);
}
#endif
