 ...
 *
 *	@(#)route.c 1.4 86/12/23 SMI; from UCB 7.1 6/4/86
 */
	...
	register struct route *ro;
{
	...
	int (*match)();
	register int doinghost;
	struct afhash h;
	u_int af = dst->sa_family;
	register struct rtentry *rtmin;
	struct mbuf **table;
...
again:
	for (m = table[hash % RTHASHSIZ]; m; m = m->m_next) {
		...
		if (doinghost) {
			if (bcmp((caddr_t)&rt->rt_dst, (caddr_t)dst,
			    sizeof (*dst)))
				continue;
			 /* With hosts we must look at all routes to find best */
			...
			if (rt->rt_dst.sa_family != af ||
				...
				continue;
			/*
			 * First matching entry should be the only entry.
			 * Therefore, it is the best entry.
			 */
			rtmin = rt;
			break;
		}
	...
	/*
	 * If we have not found an entry, maybe change what we are looking for.
	 */
	if (rtmin == 0) {
		if (doinghost) {
			doinghost = 0;
		 ...
		 */
		rtstat.rts_wildcard--;  /* undo the above increment */
		rtstat.rts_unreach++;
	} else {
		rtmin->rt_refcnt++;
	}
...
#define	equal(a1, a2) \
	 ...
	 * going down recently.
	 */
	if ((rt && !equal(src, &rt->rt_gateway)) || if_ifwithaddr(gateway)) {
		rtstat.rts_badredirect++;
		if (rt)
			rtfree(rt);
		return;
	}
	if (equal(gateway, dst)) {
		return;
	}
	/*
	...
	if (rt->rt_flags & RTF_GATEWAY) {
		if (((rt->rt_flags & RTF_HOST) == 0) && (flags & RTF_HOST)) {
			 ...
			 */
			rtinit(dst, gateway, flags | RTF_DYNAMIC);
	...
	register struct rtentry *entry;
{
	...
	case SIOCADDRT:
		...
		ifp = if_ifwithdstaddr(&entry->rt_dst);
		if (ifp == 0) {
			ifp = if_ifwithdstaddr(&entry->rt_gateway);
			if (ifp == 0) {
