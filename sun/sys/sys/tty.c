/*	@(#)tty.c 1.1 86/09/25 SMI; from UCB 6.7 83/10/01	*/
	...
	caddr_t data;
{
	...
	/*
	 * Set/get extra mode word.
	 */
	case TIOCSETX:
		tp->t_xflags = *(int *)data;
		break;

	case TIOCGETX:
		*(int *)data = tp->t_xflags;
		break;
	...
	case TIOCSWINSZ:
		if (tp->t_nlines != ((struct winsize *)data)->ws_row
		    || tp->t_ncols != ((struct winsize *)data)->ws_col) {
			tp->t_nlines = ((struct winsize *)data)->ws_row;
			tp->t_ncols = ((struct winsize *)data)->ws_col;
	...
	case TIOCGWINSZ:
		((struct winsize *)data)->ws_row = tp->t_nlines;
		((struct winsize *)data)->ws_col = tp->t_ncols;
		((struct winsize *)data)->ws_xpixel = 0;
		((struct winsize *)data)->ws_ypixel = 0;
		break;
	...
	register struct tty *tp;
{
	...
	if (tp->t_line == NTTYDISC) {
		...
		if ((tp->t_xflags&NOISIG) == 0 && c == tp->t_suspc) {
			if ((t_flags&NOFLSH) == 0)
				ttyflush(tp, FREAD);
			ttyecho(c, tp);
	...
	register struct tty *tp;
{
	...
	if (tp->t_flags & (RAW|LITOUT) || tp->t_xflags & NOPOST) {
...
loop:
	...
	while ((c = getc(qp)) >= 0) {
		 ...
		 */
		if (tp->t_line == NTTYDISC && (tp->t_xflags&NOISIG) == 0
		    && c == tp->t_dsuspc) {
			gsignal(tp->t_pgrp, SIGTSTP);
			if (first) {
				(void) sleep((caddr_t)&lbolt, TTIPRI);
