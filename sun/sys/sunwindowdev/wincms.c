#ifndef lint
static	char sccsid[] = "@(#)wincms.c 1.1 86/09/25";
#endif
		...
		if (w->w_flags & WF_CMSHOG)
			...
			if (!(dtop->dt_rootwin &&
				...
				rmfree((struct map *)dtop->dt_rmp,
				    ...
				    (long)(w->w_cms.cms_addr+CRM_ADDROFFSET));
	...
	struct colormapseg *cms;
{
	 ...
	 */
	for (wb = 0; wb < WINBUFS; wb++) {
		if (winbufs[wb] != NULL) {
			for (wi = 0; wi < WINALLOC_INC; wi++) {
				wtmp = winbufs[wb]+wi;
				if ((wtmp->w_flags & WF_OPEN) &&
				    (wtmp->w_desktop == dtop) &&
				    (strncmp(cms->cms_name,
				    wtmp->w_cms.cms_name, CMS_NAMESIZE) == 0) &&
				    (wtmp != w))
					return(wtmp);
			}
		}
	...
	int	size;
{
	...
	caddr_t	cmapdata = win_kmem_alloc(3*size);
	...
	long size;
{
	...
	for (expo = 1; expo <= 8 /* 2**8==256 */; ++expo) {
		if (size == (1 << expo)) {
			...
			if (addrlastok != -1)
				return(rmget(rmp, (int)size,
				    addrlastok+CRM_ADDROFFSET));
			else
				return(0);
		}
	}
	return(rmget(rmp, (int)size, 0+CRM_ADDROFFSET));
}
