#ifndef lint
static  char sccsid[] = "@(#)winioctl.c 1.10 87/03/20";
#endif
...
loadcmshog:
	...
	case WINSCREENNEW: {
		...
		wlok_init(&dtop->dt_mutexlock);
		wlok_init(&dtop->dt_displaylock);
		wlok_init(&dtop->dt_datalock);
		/*
...
UseWs:
	...
	case WINSCREENDESTROY: {
		...
		if (dtop->dt_flags & DTF_PRESENT) {
			...
			for (wnum = 0; wnum < NWIN; wnum++) {
				wbi = wnum/WINALLOC_INC;
				block_ptr = winbufs[wbi];
				if (block_ptr == WINDOW_NULL)
					continue;
				w_probe = (wnum-(wbi*WINALLOC_INC))+block_ptr;
				if ((w_probe->w_flags & WF_OPEN) &&
		...
		*/
		...
		if ((dtop->dt_flags & DTF_DBLBUFFER )  && 
			...
			if ( dtop->dt_dblcount == 0 ) {
				...
				* the foreground.
				*/
				dtop_changedisplay(dtop, PR_DBL_A, PR_DBL_B,
					TRUE);
			}
	 ...
	 */
	case WINPRINT:
		wt_printwindow(w, "WINPRINT");
		break;

	case _IO(g, 100):	/* Toggle winprintall flag */
		winprintall = !winprintall;
		break;
	...
	(void) splx(pri);
	if (winprintall && winprintexits) {
		if (winfilterlockscreen && (cmd == WINLOCKSCREEN ||
		    cmd == WINUNLOCKSCREEN))
			{}
		else
		  printf("winioctl exit dev: %X, cmd: %X, data: %X error: %D\n",
		      dev, cmd, data, *(int *)data, error);
	}
	return (error);
restart:
...
{
/* Note: I think there is a bug in this code that makes it do the wrong
thing if the buffer is not zeroed.  For now it is zeroed before the user
passes it in. */
	int	bytesneeded;
	...
	int	*empty;
{
	...
	*empty = 1;
	...
	if (error = copyin((caddr_t)cursor->cur_shape,
		...
		return (error);
	/*
	 * For imagesize, round width up to word boundary
	 * and turn word to bytes then multiply by height
	 * and depth.
	 */
	imagesize = (mpr->pr_width+15)/16*2*mpr->pr_height* mpr->pr_depth;
	...
	if (error = copyin((caddr_t)mpr_data->md_image,
	    (caddr_t)image, CUR_MAXIMAGEBYTES))
		return (error);
	*empty = 0;
	return (0);
}
