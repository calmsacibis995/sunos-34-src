#ifndef lint
static	char sccsid[] = "@(#)pc_dir.c 1.1 86/09/25 Copyr 1984 Sun Micro";
#endif
	...
	struct pcnode **pcpp;		/* result */
{
	 ...
	 */
	if (dp->pc_vn.v_flag & VROOT) {
		if (bcmp(namep, ".", 2) == 0 || bcmp(namep, "..", 3) == 0) {
			*pcpp =
			    pc_getnode(VFSTOPCFS(dp->pc_vn.v_vfsp),
	...
	register struct pcnode *pcp;
{
	...
	for (offset = 0;; offset += sizeof(struct pcdir)) {
		 ...
		 */
		boff = pc_blkoff(VFSTOPCFS(pcp->pc_vn.v_vfsp), offset);
...
printf("pc_findentry: looking for %s in dir 0x%x\n", namep, dp);
	...
	for (offset = 0;; ep++, offset += sizeof(struct pcdir)) {
		 ...
		 * Release previous if it exists.
		 */
		boff = pc_blkoff(VFSTOPCFS(dp->pc_vn.v_vfsp), offset);
		...
		/*
		 * Hidden files are do not participate in the search
		 */
		...
		if ((bcmp(fname, ep->pcd_filename, PCFNAMESIZE) == 0) &&
			...
			slotp->sl_ep = ep;
PCFSDEBUG(6)
printf("pc_findentry: found %s in dir 0x%x\n", namep, dp);
			return (0);
		}
	}
	if (slotp->sl_bp != NULL) {
