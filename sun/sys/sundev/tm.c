#ifndef lint
static	char sccsid[] = "@(#)tm.c 1.1 86/09/25 Copyr 1986 Sun Micro";
#endif
...
#define	MTUNIT(dev)	(minor(dev)&03)
	...
	int ctlr;
{
	...
	if (tmscp == NULL && 
	    rmget(iopbmap, sizeof (struct tmscp), (int)scp) == 0) {
	...
	if (tpb->tm_stat.tms_error || (tpb->tm_rcount & TMOP_ISSWAB) == 0) {
		if (swab == -1) {	/* already saw TapeMaster A */
	...
	int flag;
{
	...
	if (mtunit>=NMT || (sc = &mt_softc[mtunit])->sc_openf ||
	    (md = tmdinfo[mtunit]) == 0 || md->md_alive == 0)
		return (ENXIO);
	sc->sc_openf = 1;
