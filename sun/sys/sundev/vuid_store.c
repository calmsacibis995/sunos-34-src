#ifndef	lint
static	char sccsid[] = "@(#)vuid_store.c 1.1 86/09/25 Copyr 1985 Sun Micro";
#endif
...
#ifdef	KERNEL
#include "../h/systm.h"		/* For kmem_alloc */
#define	vuid_alloc(bytes)	kmem_alloc((bytes))
	...
	register Firm_event *event;
{
	...
	register u_short offset = vuid_id_offset(event->id);
	register u_short pair = event->pair;
	...
	u_short id;
{
	...
	register Vuid_seg *seg;
	Vuid_value *val_node;
	register u_short offset = vuid_id_offset(id);
