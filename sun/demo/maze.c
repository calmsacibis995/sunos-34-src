...
#ifndef lint
static  char sccsid[] = "@(#)maze.c 1.4 86/12/29 Copyr 1986 Sun Micro";
#endif
...
initialize_window() /* take over and initialize window */
{
	char cmsname[CMS_NAMESIZE];
        gfx = gfxsw_init(0,0);
	sprintf( cmsname, "maze%%10D", getpid());
	colorwindows = 0;
	if (gfx->gfx_pixwin->pw_pixrect->pr_ops == &gp1_ops)
		colorwindows = 1;
	if (gfx->gfx_pixwin->pw_pixrect->pr_ops == &cg1_ops)
		colorwindows = 1;
	if (gfx->gfx_pixwin->pw_pixrect->pr_ops == &cg2_ops)
		colorwindows = 1;
        pw_setcmsname(gfx->gfx_pixwin, cmsname);
	if (colorwindows)
		pw_putcolormap(gfx->gfx_pixwin, 0, COLOR_MAP_SIZE, rmap, gmap, bmap);
	else
