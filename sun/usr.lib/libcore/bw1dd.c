#ifndef lint
static char sccsid[] = "@(#)bw1dd.c 1.4 86/12/24 Copyr 1985 Sun Micro";
#endif
...
bw1dd(ddstruct) register ddargtype *ddstruct;
{
	...
	case INITIAL:
		...
		else if ((fbfd = _core_getfb(bwonedev, FBTYPE_SUN1BW,
		    ddstruct->int1)) >= 0)
			strncpy(screen.scr_fbname, bwonedev, SCR_NAMESIZE);
		else
		    {
		    ddstruct->logical = FALSE;
		    return( 1);
	...
	case CLEAR:
		...
		pr_rop(ip->screen,0,0,1024,800,PIX_CLR,0,0,0); /*white screen */
		if (ip->wind.needlock)
	...
	case LINE:
		...
		if ( ip->linestyle || (ip->linewidth > 1)) 
		    ...
		    if (ip->wind.needlock)
			pw_lock(ip->wind.pixwin, &ip->wind.rect);
		    pr_vector(ip->screen,ip->ddcp[0],1023-ip->ddcp[1],i1,
