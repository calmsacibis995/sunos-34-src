#ifndef lint
static char sccsid[] = "@(#)cg2dd.c 1.1 86/09/25 Copyr 1985 Sun Micro";
#endif
...
FORTNAME(cg2dd)
PASCNAME(cg2dd)
cg2dd(ddstruct) register ddargtype *ddstruct;
{
	...
	case INITZB:				/* init zbuffer */
		if ( !_core_init_pwzbuffer( ip->screen->pr_width,
		    ...
		    ip->openzbuffer = TRUE;
		} else printf("can't init zbuffer\n");
		break;
	case TERMZB:
		if (ip->openzbuffer) {
	...
	case SETZBCUT:
		...
		break;
	case INITIAL: {
		static char cgtwodev[] = "/dev/cgtwo0";
		...
		else if ((fbfd = _core_chkdev("/dev/fb", FBTYPE_SUN2COLOR,
		    ddstruct->int1)) >= 0)
			strncpy(screen.scr_fbname, "/dev/fb", SCR_NAMESIZE);
		else if ((fbfd = _core_getfb(cgtwodev,FBTYPE_SUN2COLOR,
		    ddstruct->int1)) >= 0)
			strncpy(screen.scr_fbname, cgtwodev, SCR_NAMESIZE);
		else {
		    ddstruct->logical = FALSE;
		    return( 1);
		    }
