#ifndef lint
static char sccsid[] = "@(#)cg1dd.c 1.1 86/09/25 Copyr 1985 Sun Micro";
#endif
...
#define devscale(x_ndc_coord, y_ndc_coord, x_dev_coord, y_dev_coord)	\
	if (ip->fullx){							\
		x_dev_coord = (x_ndc_coord + (x_ndc_coord >> 2)) >> 6;	\
		y_dev_coord = ip->maxy - ((y_ndc_coord +		\
			(y_ndc_coord >> 2)) >> 6) + ip->yoff;		\
	} else if (ip->fully) {						\
		x_dev_coord = (((x_ndc_coord << 4) - x_ndc_coord) >> 10)\
			+ ip->xoff;					\
		y_dev_coord = ip->maxy - (((y_ndc_coord << 4) -		\
			y_ndc_coord) >> 10);				\
	} else {							\
...
FORTNAME(cg1dd)
PASCNAME(cg1dd)
cg1dd(ddstruct) register ddargtype *ddstruct;
{
	...
	case SETZBCUT:
		...
		break;
	case INITIAL: {
		static char cgonedev[] = "/dev/cgone0";
		...
		win_initscreenfromargv(&screen, 0);
		if (*surfp->vsurf.screenname != '\0') {
			if ((fbfd = _core_chkdev(surfp->vsurf.screenname,
			    FBTYPE_SUN1COLOR, ddstruct->int1)) >= 0)
				strncpy(screen.scr_fbname,
				    surfp->vsurf.screenname, SCR_NAMESIZE);
			else if ((fbfd = _core_chkdev(surfp->vsurf.screenname,
			    FBTYPE_SUN2COLOR, ddstruct->int1)) >= 0)
				strncpy(screen.scr_fbname,
				    surfp->vsurf.screenname, SCR_NAMESIZE);
			else {
		...
		else if ((fbfd = _core_chkdev("/dev/fb", FBTYPE_SUN1COLOR,
		    ddstruct->int1)) >= 0)
			strncpy(screen.scr_fbname, "/dev/fb", SCR_NAMESIZE);
		else if ((fbfd = _core_getfb(cgonedev,FBTYPE_SUN1COLOR,
		    ddstruct->int1)) >= 0)
			strncpy(screen.scr_fbname, cgonedev, SCR_NAMESIZE);
		else {
		    ddstruct->logical = FALSE;
		    return( 1);
		    }
		...
		_core_standardcmap(ip->cmsname,0,&cmap);
		...
		break;
	case TERMINATE:
		if (ip->pf != _core_defaultfont) pf_close( ip->pf);
...
/*		printf("setndc fullxy %d %d offxy %d %d scale %d \n",
	...
	case CLEAR:
		if (ip->wind.needlock)
			pw_lock(ip->wind.pixwin, &ip->wind.rect);
		pr_rop(ip->screen,0,0,ip->screen->pr_width,
			ip->screen->pr_height, PIX_CLR,0,0,0);	/*zero screen*/
		if (ip->wind.needlock)
	...
	case SETWIDTH:
		if (ip->fullx) {
			i = ddstruct->int1;
			ip->linewidth = (i+(i>>2)+32)>>6;
		} else if (ip->fully) {
			i = ddstruct->int1;
			ip->linewidth = ((i<<4)-i+512)>>10;
		} else {
	...
	case LINE:
		...
		if ( ip->linestyle || (ip->linewidth > 1)) 
			...
			if (ip->wind.needlock)
				pw_lock(ip->wind.pixwin, &ip->wind.rect);
			pr_vector(ip->screen,ip->ddcp[0],ip->ddcp[1],
	...
	case SETUP:				/* text up direction */
		...
		vecscale( ip->ddup.x, ip->ddup.y, i1, i2);
		ip->ddup.x =  i1;
		ip->ddup.y =  -i2;
	...
	case SETPATH:				/* text path direction */
		...
		vecscale( ip->ddpath.x, ip->ddpath.y, i1, i2);
		ip->ddpath.x =  i1;
		ip->ddpath.y =  -i2;
	...
	case SETSPACE:				/* text leading << 5*/
		...
		vecscale( ip->ddspace.x, ip->ddspace.y, i1, i2);
		ip->ddspace.x =  i1;
		ip->ddspace.y =  -i2;
		break;
	...
	case TEXT:				/* raster text */
		...
		if (ip->wind.needlock)
			pw_lock(ip->wind.pixwin, &ip->wind.rect);
		pf_ttext( subpr,x,y,i ,ip->pf,ddstruct->ptr1);
		if (ip->wind.needlock)
			pw_unlock(ip->wind.pixwin);
		pr_destroy( subpr);
		break;
	case MARKER:
		...
		if (ip->wind.needlock)
			pw_lock(ip->wind.pixwin, &ip->wind.rect);
		pf_ttext( subpr,x,y,i ,_core_defaultfont,ddstruct->ptr1);
		if (ip->wind.needlock)
			pw_unlock(ip->wind.pixwin);
		pr_destroy( subpr);
		break;
	case POLYGN2: {
		...
		i1 =  ddstruct->int1;           /* vertex count */
                ...
                pr_polygon_2( (ip->wind.pixwin)->pw_pixrect, 0, 0, 
			1, &(ddstruct->int1), prvptr, i2, 0,0,0);
	    }
            break;
	case POLYGN3:
		i1 =  ddstruct->int1;		/* vertex count */
	...
	case RASPUT:
		...
		if (ip->wind.needlock)
			pw_lock(ip->wind.pixwin, &ip->wind.rect);
		pr_rop( ip->screen, x, y, width, height, i|PIX_DONTCLIP,
			&mprect, xs, ys);
		if (ip->wind.needlock)
			pw_unlock(ip->wind.pixwin);
		break;
