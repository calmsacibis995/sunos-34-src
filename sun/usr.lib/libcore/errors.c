#ifndef lint
static char sccsid[] = "@(#)errors.c 1.1 86/09/25 Copyr 1985 Sun Micro";
#endif
...
_core_errhand(funcname,errnum) char *funcname; int errnum;
{
...
/* REMEMBER to update defined constant NMSG when adding messages! */
    ...
    * 0 IS NOT A LEGAL ERROR NUMBER IN THE ROUTINE 'report_most_recent_error'
...
/****************************************************************************/
/*     FUNCTION: report_most_recent_error                                   */
/*     PURPOSE: COPIES THE ERROR NUMBER FOR THE MOST RECENTLY DETECTED      */
/*              ERROR INTO 'error'.                                         */
/****************************************************************************/
report_most_recent_error(error) int *error;
{
   *error = erreport + 1;;
   erreport = -1;
   return(0);
...
/*     PURPOSE: PRINTS THE ERROR MESSAGE FOR THE GIVEN ERROR NUMBER         */
/****************************************************************************/
print_error(string, error) char *string; int error; 
{
   if (error < 1)
	fprintf(stderr, "%s:\n%s\n", string, "NO ERROR");
   else if (error <= NMSG + 1)
       _core_errhand( string, error - 1);
   else fprintf(stderr, "print_error: No such error.\n");
   return(0);
   }
