#ifndef lint
static	char sccsid[] = "@(#)tool_select.c 1.5 87/05/04 Copyr 1983 Sun Micro";
#endif
	...
	int	waitprocessesdie;
{
	...
	struct	toolsw *sw;
	int	ibitsmask, obitsmask, ebitsmask,
		ibits, obits, ebits, nfds;
	union	wait status;
	...
	do {
		 ...
		 */
		ibitsmask = 0; obitsmask = 0; ebitsmask = 0;
		...
		do {
		    ...
		    (tool->tl_flags&TOOL_SIGCHLD));
		if (ibits == 0 && obits == 0 && ebits == 0 && timer == 0) {
			/*
			 * Wait for interrupt.
			 */
			pause();
			 ...
			 */
			nfds = select(
			    NOFILE, &ibits, &obits, &ebits, timer);
			if (nfds==-1) {
				if (errno == EINTR)
					/*
...
_tool_setupselect(tio, windowfd, ibitsmask, obitsmask, ebitsmask, timer)
	...
	int	windowfd, *ibitsmask, *obitsmask, *ebitsmask;
	struct	timeval *timer;
{
	if (tio->tio_selected) {
		*ibitsmask |= tio->tio_inputmask;
		*obitsmask |= tio->tio_outputmask;
		*ebitsmask |= tio->tio_exceptmask;
		/*
...
_tool_dispathselect(data, tio, windowfd, ibits, obits, ebits, timedelta)
	caddr_t	data;
	register struct	toolio *tio;
	int	windowfd, ibits, obits, ebits;
