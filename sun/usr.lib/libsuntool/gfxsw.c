#ifndef lint
static	char sccsid[] = "@(#)gfxsw.c 1.4 87/01/07 Copyr 1984 Sun Micro";
#endif
...
int	gfxsw_selected(), gfxsw_handlesigwinch(), gfxsw_catchsigwinch(),
	    gfxsw_catchsigtstp(), gfxsw_catchsigcont();
	...
	char	**argv;
{
	...
	gfx = (struct gfxsubwindow *) (LINT_CAST(
	    calloc(1, sizeof (struct gfxsubwindow))));
	/*
	...
	if (windowfd)
		...
		gt = (struct gfx_takeover *) (LINT_CAST(
		    calloc(1, sizeof (struct gfx_takeover))));
		gt->gt_rootfd = -1;
		/*
	...
	struct	gfxsubwindow *gfx;
{
	...
	if (gt && gt->gt_selected && (gfx->gfx_flags & GFX_RESTART)) {
		 ...
		 * happens.
		 */
		gt->gt_tool->tl_io.tio_inputmask = 0;
		gt->gt_tool->tl_io.tio_outputmask = 0;
		gt->gt_tool->tl_io.tio_exceptmask = 0;
	...
	struct	gfxsubwindow *gfx;
{
	...
	gfx->gfx_pixwin->pw_prretained = mem_create(
	    ...
	    (cms.cms_size == 2)? 1: gfx->gfx_pixwin->pw_pixrect->pr_depth);
	if (!gfxisretained(gfx))
	...
	struct	timeval **timer;
{
	...
	if (gt->gt_selected)
		gt->gt_selected(gfxtakeover, ibits, obits, ebits, timer);
}
