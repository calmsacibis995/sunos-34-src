#ifndef lint
static	char sccsid[] = "@(#)image.c 1.4 87/01/07 Copyright 1985 Sun Micro";
#endif
...
{   
    ...
    struct image *im = (struct image *)LINT_CAST(calloc(sizeof(struct image),1));
    va_list valist;
	...
	va_dcl
{
	...
	if (INHERIT_VALUE(center)) {
	    int pad = width - margin2 -
		(&pf_textwidth(strlen(im->string), font, im->string))->x;
	    left_margin = pad -
...
{   
	...
	im->width = (&pf_textwidth(strlen(im->string), font, im->string))->x;
