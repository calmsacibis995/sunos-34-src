#ifndef lint
static	char sccsid[] = "@(#)panel_choice.c 1.5 87/01/07 Copyr 1984 Sun Micro";
#endif
...
Attr_avlist			avlist;
{
    ...
    */
   dp->choices = 
       (panel_image_handle) LINT_CAST(malloc(sizeof(struct panel_image)));
   panel_make_image(ip->panel->font, &dp->choices[0], IM_STRING, "", 
		    FALSE, FALSE);
   dp->choice_rects	= (Rect *) LINT_CAST(malloc(sizeof(Rect)));
   ...
   dp->mark_rects    = (Rect *) LINT_CAST(malloc(sizeof(Rect)));
...
register panel_item_handle  ip;
{
   ...
   switch (dp->display_level) {
	 ...
	 if (dp->choose_one)
	    paint_choice(ip->panel, dp, choice_number(dp->value, dp->last));
	 else
	    /* just draw the first possible choice */
	    paint_choice(ip->panel, dp, 0);
	 break;

      case PANEL_ALL:		/* draw all the choices */
	 /* draw each choice */
	 for (i = 0; i <= dp->last; i++)
	    paint_choice(ip->panel, dp, i);
	 break;
   }
...
static
paint_choice(panel, dp, which_choice)
panel_handle     	panel;
register choice_data	*dp;
...
*/
{
   ...
   if (on) {
	 ...
	 paint_choice(ip->panel, dp, which_choice);
      }
