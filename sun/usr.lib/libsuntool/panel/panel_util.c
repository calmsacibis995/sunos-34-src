#ifndef lint
static	char sccsid[] = "@(#)panel_util.c 1.6 87/01/07 Copyr 1984 Sun Micro";
#endif
...
panel_paint_image(panel, image, rect)
panel_handle		panel;
...
Rect 			*rect;
{
   ...
   switch (image->im_type) {
	 ...
	 (void)panel_pw_text(panel, rect->r_left, top, PIX_SRC, 
      ...
      case IM_PIXRECT:
         (void)panel_pw_write(panel, rect->r_left, rect->r_top,
		        ...
		        image_pixrect(image)->pr_height,
	                PIX_SRC, image_pixrect(image), 0, 0);
	 break;
   }
...
panel_paint_pixrect(panel, pr, rect)
panel_handle	panel;
Pixrect 	*pr;
Rect    	*rect;
{
