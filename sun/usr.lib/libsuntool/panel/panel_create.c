#ifndef lint
static	char sccsid[] = "@(#)panel_create.c 1.7 87/01/07 Copyr 1984 Sun Micro";
#endif
   ...
   if (panel->event_proc) {
      ...
      if (!ops_set(ip)) {
	   new_ops = (panel_ops_handle) 
	       LINT_CAST(malloc(sizeof(struct panel_ops)));
	   *new_ops = *ip->ops;
	   ip->ops = new_ops;
	   ip->flags |= OPS_SET;
	}
...
int 		windowfd;
{
   ...
   if (defaults_get_boolean("/SunView/Walking_Menus", (Bool)(LINT_CAST(FALSE)),
   		(int *) NULL))
      panel->status        |= USING_WALK_MENUS;	
...
panel_handle panel;
{
   ...
   win_setinputcodebit(&mask, KEY_NEXT);
   win_setinputcodebit(&mask, PUT_KEY);
   win_setinputcodebit(&mask, GET_KEY);
   win_setinputcodebit(&mask, FIND_KEY);
   win_setinputcodebit(&mask, DELETE_KEY);
   win_setinputcodebit(&mask, KEY_LEFT(4));
   win_setinputcodebit(&mask, KBD_USE);
   win_setinputcodebit(&mask, KBD_DONE);
   (void)win_set_kbd_mask(panel->windowfd, &mask);
   /* Initialize and set kbd input mask */
   (void)input_imnull(&mask);
   mask.im_flags |= IM_NEGEVENT;
