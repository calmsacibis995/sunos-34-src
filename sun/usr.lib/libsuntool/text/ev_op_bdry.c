#ifndef lint
static  char sccsid[] = "@(#)ev_op_bdry.c 1.18 87/03/17";
#endif
	...
	Ev_mark			 id_handle;
{
	...
	if (!EV_MARK_MOVE_AT_INSERT(*id_handle)) {
	    for (seq = FT_PREV_ADDR(seq, addr_delta); index > 0; index--) {
		if (*seq != pos)
		    break;
...
ev_op_bdry_info_merge(op_bdry, i, next_i, prior)
	...
	unsigned		 prior;
{
	Op_bdry_handle		 seq = (Op_bdry_handle) op_bdry.seq;
	int			 pos = seq[i].pos;
	unsigned		 result = prior;
...
ev_add_glyph(chain, pos, pr, op, offset_x, offset_y)
	Ev_chain	 chain;
	Es_index	 pos;
	struct pixrect	*pr;
	int		 op;
	...
	int		 flags;
{
	...
	&& !(flags & EV_GLYPH_WORD_START)) {
	    ...
	    (void) ev_span(chain, line_start, &first, &pos,
