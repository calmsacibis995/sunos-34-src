#ifndef lint
static  char sccsid[] = "@(#)textsw_again.c 1.4 87/01/07";
#endif
...
char		*cmd_tokens[] = {
	"DELETE", "EDIT", "FILTER", "FIND", "INSERT", "MENU", NULL
};
...
textsw_string_append(ptr_to_string, buffer, buffer_length)
	...
	char		*buffer;
	int		 buffer_length;
/* Returns FALSE iff it needed to calloc and the calloc failed */
{
...
/* Returns FALSE iff it needed to calloc and the calloc failed */
{
	...
	if (ptr_to_string->max_length < desired_max) {
	    ...
	    ptr_to_string->base = calloc((unsigned)desired_max, sizeof(char));
	...
	int		 pattern_length, direction;
{
	...
	if (textsw->func_state & TXTSW_FUNC_AGAIN)
	    return;
	if (textsw->state & TXTSW_AGAIN_HAS_FIND) {
	...
	Event			*event;
{
	...
	(void) textsw_printf(again, "%s %x %x %x ",
			     cmd_tokens[ord(FILTER_TOKEN)], event->ie_code,
...
#define CHECK_ERROR(test)	if AN_ERROR(test) goto Again_Error;
	...
	CHECK_ERROR(*TXTSW_STRING_BASE(again)++ != '\n');
	if (length) {
	    *ptr_to_buffer = calloc((unsigned)length, sizeof(**ptr_to_buffer));
