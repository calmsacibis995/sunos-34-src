#ifndef lint
static  char sccsid[] = "@(#)ev_attr.c 1.6 87/03/17";
#endif
	...
	caddr_t			 args1, args2, args3;
{
	...
	switch (attribute) {
	  ...
	  case EV_DISPLAY_START:
	    return((caddr_t)view->line_table.seq[0]);
	  case EV_LEFT_MARGIN:
	...
	FORALLVIEWS(chain, next) {
		...
		switch (to) {
		  case EV_START_CURRENT_POS:
		    (void) ev_view_range(next, &first, &last_plus_one);
		...
		(void) ev_display_view(next);
	    } else {
		if (incremental) {
		    (void) ev_set_start(next, to);
		} else {
				...
				*intersect_rect;
{
	...
	if (view->line_table.seq[0] != ES_INFINITY)  {  
	...
	(void) ft_expand(&view->line_table, lines - old_lines);
	if (intersect_rect &&
	    rect_intersectsrect(intersect_rect, &view->rect)) {
	    (void) ev_display_in_rect(view, intersect_rect);
	}
}
