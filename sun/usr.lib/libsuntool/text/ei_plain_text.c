#ifndef lint
static  char sccsid[] = "@(#)ei_plain_text.c 1.6 87/03/17";
#endif
...
#ifdef DEBUG
	(void) fprintf(stderr, "Font_flags: %X\n", private->font_flags);
#endif
}
...
ei_plain_text_process(eih, op, esbuf, x, y, rop, pw, rect, tab_origin)
	...
	int			 tab_origin;
/*
 *   Note that y is the position of the largest ascender's top,
 * NOT the baseline.
...
#undef	bounds_right
	...
	if (pw->pw_prretained) {
	    ...
	    prpos.pos.y = y;
	    if (batch_length > 0)
		(void) prs_batchrop(prpos, rop, batch, batch_length);
	} else {
		...
		if (batch_length > 0) {
		    if (rect_includesrect(&rclip, bounds)) {
			    (void) prs_batchrop(prpos, rop|PIX_DONTCLIP,
				    batch, batch_length);
			    break;
		    } else
			   (void) prs_batchrop(prpos, rop&(~PIX_DONTCLIP),
