#ifndef lint
static  char sccsid[] = "@(#)finger_table.c 1.10 87/03/17";
#endif
	...
	int		last_plus_one, sizeof_client_data;
{
	...
	result.seq = (Es_index *) LINT_CAST(calloc(
		(unsigned)last_plus_one, result.sizeof_element));
...
ft_expand(table, by)
	...
	int			 by;
{
	Es_index		*old_seq = table->seq;
	int			 old_last_plus_one = table->last_plus_one;

	table->last_plus_one += by;
	table->seq = (Es_index *) LINT_CAST(malloc(
	    (unsigned)table->last_plus_one * table->sizeof_element));
	bcopy((char *)old_seq, (char *)table->seq,
	    (int)table->sizeof_element*(old_last_plus_one+(by<0 ? by : 0)));
	if (by > 0) {
	    (void) ft_set(*table, old_last_plus_one, table->last_plus_one,
		ES_INFINITY, (char *)0);
	}
	free((char *)old_seq);
}
	...
	int		first, last_plus_one;
{
	...
	if (last_plus_one < table->first_infinity) {
	    ...
	    to_move = 0;
	(void) ft_set(*table, first+to_move, table->first_infinity,
		ES_INFINITY, (char *)0);
	...
	char			*client_data;
{
	...
	(void) ft_set(finger_table, index_of_first, index_of_last_plus_one,
		to, client_data);
}
	...
	ft_object	finger_table;
{
	...
	(void) fprintf(out_file,
		       "last_plus_one = %d, sizeof_element = %d, seq = 0x%X\n",
