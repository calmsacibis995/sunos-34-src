#ifndef lint
static  char sccsid[] = "@(#)textsw_attr.c 1.22 87/03/17";
#endif
...
static Es_index	textsw_get_contents();
	...
	int			 all_views;
{
	...
	if (filename) {
		...
		(void) textsw_format_load_error(msg, load_status,
	    ...
	    if (first != ES_CANNOT_SET) {
	       if (all_views) {
	         Textsw_view   view_ptr;
	         for (view_ptr = folio->first_view; view_ptr; view_ptr = view_ptr->next) {
	         	(void) textsw_normalize_internal(view_ptr, first, first, 0, 0,
				  TXTSW_NI_DEFAULT);     
	         };
...
TLBA_Tail:
	...
	if (display_views && (textsw->state & TXTSW_DISPLAYED)) {
		...
		(void) ev_display_view(next->e_view);
	    }
	...
	va_dcl
{
	...
	switch (attribute) {
	  ...
	  case TEXTSW_FIRST:
	    return((caddr_t)view->e_view->line_table.seq[0]);
	  case TEXTSW_FIRST_LINE: {
	    int	top, bottom;
