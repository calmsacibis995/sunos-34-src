#ifndef lint
static	char sccsid[] = "@(#)menu.c 1.4 87/01/07 Copyr 1984 Sun Micro";
#endif
...
struct	cursor menu_cursor = { 17, 6, PIX_SRC|PIX_DST, &menu_mpr};
...
Repaint:
	...
	if (promotemenuno) {
		...
		promotemenuno = 0;
		if (!(_menu_event->ie_code == _menu_butcode &&
	...
	struct	inputevent *initevent;
{
	...
	_menu_butcode = initevent->ie_code;
	/*
	 * Open font
	 */
	...
	for (menu=(menu_list);menu;menu=menu->m_next) {
		...
		if (menu->m_data==0) {
			menu->m_data =  (caddr_t) (LINT_CAST(
				calloc(1, sizeof (struct menudata))));
		}
		/*
