#ifndef lint
static char  sccsid[] = "@(#)icon_load.c 1.5 87/03/17 SMI";
#endif
	...
	struct pixrect *pr;
{
	...
	if ((header->valid_bits_per_item != 16 &&
		...
		MP_NOTMPR(pr) || mprd->md_flags & MP_REVERSEVIDEO ||
	 ...
	 * a scratch buffer for the data as it is read; otherwise
	 * read it directly into the pixrect image.
	 */
	if (linepad) { 
	 ...
	 */
	{
		...
		register long_items = (header->valid_bits_per_item > 16);

		for (count /= 2; count; count--) {
			register int c;
			long value;

			c = fscanf(fd, " 0x%X,", &value);
			if (c == 0 || c == EOF)
				break;

			if (long_items) {
				* (long *) p = value;
				p += (sizeof value / sizeof *p);
			}
			else 
				*p++ = value;
		}
	...
	/*
	 * Copy image from scratch buffer to pixrect if necessary.
	 */
	if (linepad) {
		register short *in, *out;
		register int w, h;

		in = image;
		out = mprd->md_image;

		linebytes /= sizeof *out;
		linepad /= sizeof *out;

		for (h = header->height; h; h--) {
			for (w = linebytes; w; w--)
				*out++ = *in++;
			out += linepad;
		}
...
/* See comments in icon_load.h */
{
	...
	if (result == NULL_PIXRECT) {
	    (void)sprintf(error_msg, 
	    	"Cannot create memory pixrect %dx%dx%d.\n",
