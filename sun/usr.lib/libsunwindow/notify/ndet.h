...
extern	u_int	ndet_fndelay_mask;	/* Mask of non-blocking read fds
					   (maintained by fcntl) */
extern	u_int	ndet_fasync_mask;	/* Mask of fds that generate SIGIO
					   when data ready (maintained by
					   fcntl) */

extern	int	ndet_ibits, ndet_obits, ndet_ebits;
...
extern	ndet_tv_equal();		/* Returns 1 if a and b are equal
					   (struct timeval a, b) */
...
typedef struct ndet_enum_send {
	int	ibits;			/* Input devices selected */
	int	obits;			/* Output devices selected */
	int	ebits;			/* Exception devices selected */
