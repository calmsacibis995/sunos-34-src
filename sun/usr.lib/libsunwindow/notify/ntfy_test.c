#ifndef	lint
static	char sccsid[] = "@(#)ntfy_test.c 1.4 87/01/07 Copyr 1985 Sun Micro";
#endif
...
/*
  Not yet tested:
	...
	notify_get_exception_func
		Set up tcp connection and use SIGURG

	notify_output
	notify_exception
	notify_wait3
		Used by default prioritizer.  Need to do partial notify
		of conditions and let default do the rest.

  Program input data:
 	Startup parameters:
 		Duration: Less of two limits
 			Time: -t(ime) nsecs (default - 20)
 			Actions Processed: -a(ctions) n
				(default - infinite)
		Output: (stdout)
			Verbose: -v(erbose) on/off (default - off)
		Errors: (stderr)
		Unix event generation speed: -s(peed) fast/med/slow
				(default med)
			Controls speed of generating signals and fd activity.
			After posting an event gotten out of an action file
			(as test process):
				fast: Don't delay
				med: Sleep 1 second
				slow: Sleep 5 second
			After posting an event gotten out of an action file
			(as exerciser process):
				fast: Delay 1/10th of a second
				med: Sleep 1 second
				slow: Sleep 5 second
		Client-on-stack: -i(nput) none/read/select (default - select)
			none: Simply call notify_start
			read: Use read only
			select: Use select and read
		Clients: -c(lient) random/filename/fixed n
				(default - -c random 2 -c fixed 2)
				(filename convention: nts.*)
			random: Generate condition/events randomly
			filename: Action file to determine initial setup
				and condition changes and event generation
			fixed: Built-in action file
		Child process exerciser: -x wait3/filename
			(default - wait3) (filename convention: nte.*)
			wait3: Change state repetitively
			filename: Get SIGNAL events out of this
			    actions file (see below) and send to parent process
			This process forked from notifier test process and
			   should serve as an exerciser
 	Client action file format:
		CONDITION cycle_count (no cycle_count means infinite)
			Cycle_count is the number of times that will go thru
			a condition before becoming simply NONE.
		action (see below)
		action (see below)
		...
		CONDITION
		...
		Each condition section serves as a mini-script of condition
		change commands and event generation commmand.  The section
		has 2 implicit indices that chase around the section; one
		for the current condition and one for the current event.
		The indices are reevaluated whenever the condition is notified.

		Reevaluation involves incrementing the current event index
		until an unknown event is found.  Valid event lines generate
		actual events during this search.  There is a test to prevent
		the same event from being executing multiple times in one sweep.

		Next, the current event index in incremented until an known
		condition is found.  A valid condition line sets an actual
		condition.  There is a test to prevent unnecessarily
		resetting the existing condition to itself.

		When have an exerciser script, change condition commands are
		not allowed.

		When have a tester script, single events (without a condition)
		are not allowed.

		actions:
			change condition:
				INPUT pipe# [async] [ndelay]
					Default is normal blocking.
				OUTPUT pipe# Tag
				EXCEPTION pipe# block/async (???)
				REAL seconds useconds tag
					(Multiple reals not allowed in client.
					 If do, first one [in the action list]
					 is the one detected as going off).
				VIRTUAL seconds useconds tag
					(See REAL).
				ASYNC signal_name
				SYNC signal_name
				WAIT3 program_name args
				    (e.g., ntfy_tchild -x wait3)
					(If child stopped, will do next action
					 only if event so wouldn't abandon child
					 process).
				DESTROY
					(Once death or cleanup received the
					 client and all its conditions are
					 history because the notifier axed
					 the client completely.  Follow by
					 NONE.  However, SEND_DESTROY is
					 still alive because notification
					 happens immediately).
				VETOER
					(See DESTROY).
				SAFE
				IMMEDIATE
				NONE
			generate event: (Doesn't affect current condition).
				WRITE pipe# string
					(Companion to INPUT).
				READ pipe#
					(Companion to OUTPUT).
				SIGNAL signal_name
					(If no condition waiting then might
					 blow process out of water if default
					 action is to core dump).
				ITIMER_REPORT real/virtual
					(Reports current itimer value).
					(Shouldn't report real/virtual itimer
					 immediately after getting such a
					 notification because the condition
					 will be undefined).
				SEND_DESTROY death/checking/cleanup
					(See DESTROY).
				SEND_SAFE string
					(If use string greater than one char
					 then can expect "NOTIFICATION
					 UNEXPECTED: SAFE" messages when stop
					 from keyboard).
				SEND_IMMEDIATE string
					(See SEND_SAFE).
				EXERCISER program_name args
					(e.g., ntfy_tchild -x nte.*)
					(Follow this with NONE condition).
				NOTHING tag
 */
...
int	exerciser;	/* Generating Unix activity as a separate process */
int	wait3er;	/* Acting as child process to tester */

int	test_pid;	/* Test process pid (can be multiple exerciser and/or
			   wait3er child process) */
int	terminated;	/* SIGTERM received by wait3er or exerciser */
int	nt_errors;	/* Number of non-fatal (didn't cause an exit) errors */
...
enum	generation_speed {
	...
	MEDIUM=1,
	SLOW=2,
} generation_speed = FAST;	/* When exerciser then default is MED */
...
#\tRemove numbered client\n");
			...
			case 'q':	/* Quit */
				...
				break;
			case 's':	/* Halt any further condition setting
					   so will expire gracefully soon */
				set_conditions = 0;
				fprintf(stderr,
	...
	int which;
{
	...
	if (verbose && which == ITIMER_REAL)
		fprintf(stdout, "	itimer error sec=%D, usec=%D\n",
	...
	Notify_signal_mode mode;
{
	...
	/*
	 * NOTE: DANGER, doing things when mode == async can screw up
	 *	ntfy_test internal data structures.
	 */
	/* Do next action (may or may not unset current condition) */
	nt_next_action(client, cond);
	return(NOTIFY_DONE);
	...
	struct	rusage *rusage;
{
	...
	if (WIFEXITED(*status)) {
		...
		if (notify_get_wait3_func(client, pid) != NOTIFY_FUNC_NULL)
			...
			/*
			 * Tell nt_unset_condition to clear current_condition
		...
		nt_reset_ntfy_msg();
		if (verbose)
			fprintf(stdout, "CHILD SIGNALED sig = %D\n",
			    status->w_termsig);
	} else if (WIFSTOPPED(*status)) {
		char *cur_cond;

		if (verbose)
			fprintf(stdout, "CHILD STOPPED sig = %D\n",
			    status->w_stopsig);
		...
		if (!set_conditions) {
			...
			if (kill(pid, SIGCONT) == -1)
				perror("KILL SIGCONT");
			/*
			 * Do next action only if it is an event,
			 * because can't abandon child process
			 */
	...
	Destroy_status status;
{
	...
	case DESTROY_PROCESS_DEATH:
		/*
		 * Tell nt_unset_condition to clear current_condition
		 * without unsetting it.  Notifier will unset it.
		 */
		cond->flags |= CD_NO_UNSET_COND;
		break;
	default:
	...
	CONDITION *cond;
{
	if (cond->current_condition != NULL) {
		...
		fprintf(stderr, "[%D] registered condition: %s\n",
	...
	CONDITION *cond;
{
	...
	/* Get next action */
	cond->current_action = index(cond->current_action, NULL);
	...
	register CONDITION *cond;
{
	...
	if (strcmp(token, "WRITE") == 0) {
		...
		if (sscanf(arg1, "pipe%D", &pipe_id) != 1 ||
		    pipe_id >= MAX_PIPES) {
		...
		if (verbose)
			fprintf(stdout, "[%D] %s: ", cn, cond->current_action);
		if (n < 3) {
			arg2[0] = 'a';
			arg2[1] = NULL;
		}
	...
	CONDITION *cond;
{
	...
	if (strcmp(token, "INPUT") == 0 && n > 1) {
		...
		/* See what pipe pair using */
		if (sscanf(arg1, "pipe%D", &cond->data.pipe) != 1 ||
...
#define	ARGS_MAX        100
	...
	/*
	 * Give child process chance to get thru execvp so as to
	 * avoid (reduce the likelyhood of) a race with the parent
	 * process while debugging the parent.  Supposedly, sleep
	 * resets the SIGALRM handler so we would mess up the notifier.
	 */
	sleep(2);
	return(pidchild);
}
	...
	CONDITION *cond;
{
	...
	if (old_func == NOTIFY_FUNC_NULL) {
		fprintf(stderr, "Unset_func error %D on %s\n", notify_errno,
	...
	int data, use;
{
	...
	for (i = 0; i < client->cond_next; i++) {
		...
		if (verbose)
			fprintf(stdout, "[%D] Notification Received: %s\n",
			    (client-&clients[0]), cond->current_condition);
		return(cond);
	}
	...
	while (argc > 0 && argv[0][0] == '-' && argv[0][1]) {
		...
		case 't':	/* Set time limit */
			...
			if (sscanf(argv[1], "%hD", &halt_itimer.it_value.tv_sec)
				...
				goto ArgFormat;
			/*
			 * Set interval so that when unset itimer condition 
			 * it will still be there, even though it expired once.
			 */
	...
	char *type_str;
{
	...
	if (strcmp(type_str, "random") == 0) {
		...
		clt->data = calloc(1, size+1/*For NULL*/);
...
be_exerciser()
{
	...
	if (client_next != 1 || client->cond_next == 0) {
		fprintf(stderr, "EXERCISER no client or condition %D %D\n",
