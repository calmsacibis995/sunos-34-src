#ifndef	lint
static	char sccsid[] = "@(#)ndet_auto.c 1.7 87/01/07 Copyr 1985 Sun Micro";
#endif
...
#include "ntfy.h"
#include "ndet.h"
#include "ndis.h"	/* For ndis_enqueue */
#include <errno.h>
#include <signal.h>

extern	errno;

pkg_private_data u_int ndet_sigs_auto;
	...
	NTFY_ENUM_DATA context;
{
	...
	case SIGURG: {
		...
		while (loop_limiter++ < 5) {
			...
			nfds = notify_select(8*sizeof(ibits),
			...
			/* Send notifications */
			if (enum_send2.ibits || enum_send2.obits ||
			    enum_send2.ebits)
			 ...
			 */
			enum_send->ibits |= enum_send2.ibits;
			enum_send->obits |= enum_send2.obits;
			enum_send->ebits |= enum_send2.ebits;
			break;
		}
		break;
		}
