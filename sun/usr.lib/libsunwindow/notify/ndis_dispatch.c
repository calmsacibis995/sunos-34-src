#ifndef	lint
static	char sccsid[] = "@(#)ndis_dispatch.c 1.4 87/01/07 Copyr 1985 Sun Micro";
#endif
...
#include "ndis.h"
#include "nint.h"
#include "ndet.h"	/* For ndet_set_event_processing/ndet_flags */
#include <signal.h>

pkg_private_data u_int	ndis_flags;
pkg_private_data NTFY_CLIENT *ndis_clients;
pkg_private_data NTFY_CLIENT *ndis_client_latest;
	...
	Notify_client nclient;
{
	...
	int sigbits, auto_sigbits, ibits, obits, ebits;
...
Retry_Conditions:
	...
	sigbits = auto_sigbits = ibits = obits = ebits = 0;
	ndis_event_count = 0;
	/* Fill out prioritizer arguments based on conditions */
	for (cdn = client->conditions; cdn; cdn = cdn->next) {
		switch (cdn->type) {
		case NTFY_INPUT:
			ibits |= FD_BIT(cdn->data.fd);
			break;
		case NTFY_OUTPUT:
			obits |= FD_BIT(cdn->data.fd);
			break;
		case NTFY_EXCEPTION:
			ebits |= FD_BIT(cdn->data.fd);
			break;
	...
	NTFY_END_CRITICAL;
	(void) pri_func(nclient, sizeof(int)*8, &ibits, &obits, &ebits,
