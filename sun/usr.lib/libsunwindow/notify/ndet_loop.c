#ifndef	lint
static	char sccsid[] = "@(#)ndet_loop.c 1.6 87/01/07 Copyr 1985 Sun Micro";
#endif
...
#include "ndet.h"
#include "nint.h"
#include "ndis.h"		/* For ndis_dispatch */
#include <syscall.h>
#include <errno.h>
#include <signal.h>
...
pkg_private_data u_int	ndet_flags;
pkg_private_data NTFY_CLIENT *ndet_clients;
pkg_private_data NTFY_CLIENT *ndet_client_latest;
pkg_private_data u_int	ndet_sigs_received;
pkg_private_data u_int	ndet_sigs_managing;
pkg_private_data u_int	ndet_fasync_mask;
pkg_private_data u_int	ndet_fndelay_mask; /* Maintained, but really used? */

pkg_private_data struct	timeval ndet_polling_tv = {0,0};

pkg_private_data int	ndet_ibits, ndet_obits, ndet_ebits;
...
notify_start()
{
	...
	do {
		...
		if (ibits == 0 && obits == 0 && ebits == 0 &&
		    timer == NTFY_TIMEVAL_NULL && !ndet_sigs_received) {
			...
			nfds = syscall(SYS_select,
			    8*sizeof(ibits), &ibits, &obits, &ebits,
...
Finish:
	...
	if (ndet_flags & NDET_STOP_ON_SIG) {
		...
		sprintf(sb,
		    "errno changed (old=%D, cur=%D); reset to old",
		    errno_remember, errno);
...
ndet_fig_fd_change()
{
	...
	/* Zero out bits */
	ndet_ibits = ndet_obits = ndet_ebits = 0;
	...
	NTFY_ENUM_DATA context;
{
	...
	case NTFY_INPUT:
		if ((bit = FD_BIT(condition->data.fd)) & ndet_fasync_mask)
...
ndet_update_real_itimer()
{
	...
	n = gettimeofday(&enum_itimer.current_tv, &dzone); /* SYSTEM CALL */
...
pkg_private_data int	ndet_track_sigs;

pkg_private int	/* Should be static but there might be clients of it */
	...
	NTFY_ENUM_DATA context;
{
	...
	switch (condition->type) {
	case NTFY_INPUT:
		if (FD_BIT(condition->data.fd) & enum_send->ibits)
			goto EnQ;
		break;
	case NTFY_OUTPUT:
		if (FD_BIT(condition->data.fd) & enum_send->obits)
			goto EnQ;
		break;
	case NTFY_EXCEPTION:
		if (FD_BIT(condition->data.fd) & enum_send->ebits)
			goto EnQ;
		break;
	default: {}
	}
