#ifndef lint
static	char sccsid[] = "@(#)convert.2.c 1.1 86/09/25 Copyr 1984 Sun Micro";
#endif
...
/*
 * Standard rasterfile encode/decode filter for ras_type == RT_BYTE_ENCODED.
 */
	...
	char	**argv;
{
	 ...
	 *   read the image.
	 * If the rasterfile is our (non-standard) type, use an appropriate
	 *   routine to read the image (for RT_BYTE_ENCODED, this turns out
	 *   to also be pr_load_std_image).
	...
	case RT_STANDARD:
		if ((input_pr = pr_load_std_image(infile, &rh)) == NULL)
			SERROR(PR_IO_ERR_RASREAD);
		if ((output_pr = pr_dump_init(input_pr, &rh, &colormap,
					     MY_RAS_TYPE, FALSE)) == NULL)
			SERROR(PR_IO_ERR_RASWRITE);
		if (pr_dump_encoded(output_pr, outfile, &rh, &colormap) != 0)
			SERROR(PR_IO_ERR_RASWRITE);
		break;
	case MY_RAS_TYPE:
		if ((input_pr = pr_load_std_image(infile, &rh)) == NULL)
