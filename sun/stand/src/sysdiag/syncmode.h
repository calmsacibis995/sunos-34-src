/*      @(#)syncmode.h 1.1 86/09/25 SMI; from UCB 4.34 83/06/13      */
 ...
 * synchronous serial lines, both RS-232 and RS-449
 * This structure must be kept under 16 bytes in order to work
 * with the ifreq ioctl kludge.
 */
struct syncmode {
	char	sm_txclock;	/* enum - transmit clock sources */
	char	sm_rxclock;	/* enum - receive clock sources */
	char	sm_loopback;	/* boolean - do internal loopback */
