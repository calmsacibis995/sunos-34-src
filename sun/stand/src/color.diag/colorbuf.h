...
/*
    This file contains
    the constants used by other modules which use the color graphics
    board.   PWC 2/12/82.      

    Modified for first PC version of board. 10/20/82.
*/
...
# define Set_CFunc(op)    *GR_freg = (op)		/* Due to AM2952s */
# define Set_CMask(op)    *GR_mask = (op)		/* Due to AM2952s */
# define Read_CFunc(op)   op = (*GR_freg)		/* Due to AM2952s */
# define Read_CMask(op)   op = (*GR_mask)		/* Due to AM2952s */
...
# define GR_set		   (uchar)0xFF  /* Set bit in frame buffer */
