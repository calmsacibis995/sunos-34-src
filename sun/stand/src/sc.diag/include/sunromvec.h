...
/* Monitor and hardware revision and identification */
	...
	struct bootparam	**v_bootparam;	/* Info for bootstrapped pgm */
 	int		*v_memorysize;		/* Usable memory in bytes */
...
/* Keyboard input (scanned by monitor nmi routine) */
	...
	struct keyboard	**v_curkeyboard;	/* cur keyboard defn */
	struct keymap	*(**v_setkeytable)();	/* cur shift->table mapper */
...
/* nmi related information */
	...
	int		*v_dummy4;		/* Was: bedecode() */

/* Assorted other things */
	int		(**v_keyentry)();	/* unknown entry handler */
	...
	char		***v_keystrtab;		/* table of key strings */
...
#define	v_CurKeyboard	v_curkeyboard
#define	v_SetTable	v_setkeytable
