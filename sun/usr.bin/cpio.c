#ifndef lint
static	char sccsid[] = "@(#)cpio.c 1.1 86/09/25 SMI"; /* from S5R2 1.17 */
#endif
...
accerr:
				fprintf(stderr,"cannot write in <%s>: ", argv[2]);
				perror("");
				exit(2);
			}
		...
		case 'r':		/* rename files interactively */
			...
			if(Rtty==NULL || Wtty==NULL) {
				perror("cpio: Cannot rename (can't open /dev/tty)");
				exit(2);
			}
			break;
	...
	if(Option == PASS) {
		...
		if(Cflag) {
		    if ((Cp = Cbuf = (char *)malloc(Bufsize)) == NULL) {
			perror("cpio");
			exit(2);
		    }
		} else {
		    if ((Wp = Dbuf = (short *)malloc(Bufsize)) == NULL) {
			perror("cpio");
			exit(2);
		    }
		}
	...
	case OUT:		/* get filename, copy header and file out */
		while(getname()) {
			...
			if (One_extent || Multi_extent) {
			   ...
			   if( Cflag ) {
				writehdr(Chdr,CHARS+Hdr.h_namesize);
				writehdr(Symlbuf,symlsz);
			   } else {
				bwrite((short *)&Hdr, HDRSIZE+Hdr.h_namesize);
				bwrite((short *)Symlbuf, symlsz);
			   }
			   continue;
			}
			if((Ifile = open(Hdr.h_name, 0)) < 0) {
				fprintf(stderr,"Cannot open <%s>: ", Hdr.h_name);
				perror("");
				continue;
			}
			if ( Cflag )
			...
			for(filesz=mklong(Hdr.h_filesize); filesz>0; filesz-= CPIOBSZ){
				...
				if(read(Ifile, Cflag? BBuf: (char *)Buf, ct) < 0) {
					fprintf(stderr,"Cannot read %s: ", Hdr.h_name);
					perror("");
					continue;
				}
	...
	case IN:
		...
		while(gethdr()) {
			if (A_symlink) {
				symlsz = (int) mklong(Hdr.h_filesize);
				if(Cflag)
					readhdr(Symlbuf,symlsz);
				else
					bread((short *)Symlbuf, symlsz);
				Symlbuf[symlsz] = '\0';
				if(ckname(Hdr.h_name))
					(void)openout(Hdr.h_name, Symlbuf);
			} else {
				Ofile = ckname(Hdr.h_name)? openout(Hdr.h_name, Symlbuf): 0;
				for(filesz=mklong(Hdr.h_filesize); filesz>0; filesz-= CPIOBSZ){
					ct = filesz>CPIOBSZ? CPIOBSZ: filesz;
					...
					if(Ofile) {
						if(Swap)
						   Cflag?
						       swap(BBuf,ct,byteswap,halfswap):
						       swap(Buf,ct,byteswap,halfswap);
						if(write(Ofile, Cflag? BBuf: (char *)Buf, ct) < 0) {
						 fprintf(stderr,"Cannot write %s: ", Hdr.h_name);
						 perror("");
						 continue;
						}
					}
				...
				if(Ofile) {
					...
					if(chmod(Hdr.h_name, Hdr.h_mode) < 0) {
						fprintf(stderr,"Cannot chmod <%s>: ", Hdr.h_name);
						perror("");
					}
				...
				if(link(Hdr.h_name, Fullname) < 0) { /* missing dir.? */
					if(errno == EEXIST) {
						...
						if(link(Hdr.h_name, Fullname) < 0) {
							fprintf(stderr, "Cannot link <%s> & <%s>: ", Hdr.h_name, Fullname);
							perror("");
							continue;
						}
					}
...
/* try creating (only twice) */
				...
				if(ans == 1) {
					fprintf(stderr,"Cannot create directory for <%s>: ", Fullname);
					perror("");
					exit(0);
				}else if(ans == 2) {
					fprintf(stderr,"Cannot link <%s> & <%s>: ", Hdr.h_name, Fullname);
			...
			if (One_extent || Multi_extent)
				actsize(0);
#endif
			if(!(Ofile = openout(Fullname, Symlbuf)))
				continue;
			if((Ifile = open(Hdr.h_name, 0)) < 0) {
				fprintf(stderr,"Cannot open <%s>: ", Hdr.h_name);
				perror("");
			...
			if(Ofile) {
				...
				if(chmod(Fullname, Hdr.h_mode) < 0) {
					fprintf(stderr,"Cannot chmod <%s>: ", Fullname);
					perror("");
				}
...
getname()		/* get file name, get info for header */
{
	...
	for(;;) {
		...
		if(lstat(namep, &Statb) < 0) {
			fprintf(stderr,"Cannot stat <%s>: ", Hdr.h_name);
			perror("");
			continue;
		}
...
long t;
{
	sprintf(Chdr,"%.6o%.6ho%.6ho%.6ho%.6ho%.6ho%.6ho%.6ho%.11lo%.6ho%.11lo%s",
		...
		Statb.st_uid,Statb.st_gid,Statb.st_nlink,
		Statb.st_rdev & 00000177777,Statb.st_mtime,
...
gethdr()		/* get file headers */
{
	...
	if(Cflag)
		...
		bread((short *)Hdr.h_name, Hdr.h_namesize);
...
/* try creating (only twice) */
			...
			if(ans == 1) {
				fprintf(stderr,"Cannot create directory for <%s>: ", namep);
				perror("");
				return(0);
			}else if(ans == 2) {
				fprintf(stderr,"Cannot create directory <%s>: ", namep);
				perror("");
				return(0);
			}
		}
...
ret:
		if(chmod(namep, Hdr.h_mode) < 0) {
			fprintf(stderr,"Cannot chmod <%s>: ", namep);
			perror("");
		}
		if(Uid == 0)
			if(chown(namep, Hdr.h_uid, Hdr.h_gid) < 0) {
				fprintf(stderr,"Cannot chown <%s>: ", namep);
				perror("");
			}
	...
	if(lstat(namep, &Xstatb) == 0) {
		if(Uncond && !((!(Xstatb.st_mode & S_IWRITE) || A_special) && (Uid != 0))) {
			if(unlink(namep) < 0) {
				fprintf(stderr,"cannot unlink current <%s>: ", namep);
				perror("");
			}
		}
...
symlname, namep) < 0) {
		...
		if(ans == 1) {
			...
			fprintf(stderr,"Cannot symlink <%s> and <%s>: ", namep, symlname);
			perror("");
			return(0);
		}
...
/* try creating (only twice) */
		...
		if(ans == 1) {
			...
			fprintf(stderr,"Cannot mknod <%s>: ", namep);
			perror("");
			return(0);
		}
...
/* try creating (only twice) */
		...
		if(ans == 1) {
			...
			fprintf(stderr,"Cannot create <%s>: ", namep);
			perror("");
			return(0);
		}
...
/* try linking (only twice) */
			...
			if(ans == 1) {
				fprintf(stderr,"Cannot create directory for <%s>: ", np);
				perror("");
				return(0);
			}else if(ans == 2) {
				fprintf(stderr,"Cannot link <%s> & <%s>: ", ml[i]->m_name, np);
				perror("");
				return(0);
			}
...
chgreel(x, fl)
{
	...
	fprintf(stderr,"Can't %s: ", x? "write output": "read input");
	perror("");
	fstat(fl, &statb);
#ifndef RT
...
again:
	...
	if ((devtty = fopen("/dev/tty", "r")) == NULL) {
		perror("cpio: Cannot prompt (can't open /dev/tty)");
		exit(2);
	}
	fgets(str, 20, devtty);
	fclose(devtty);
		...
		*p_end = '\0';
		if(chdir(p_save) == -1) {
			if((ans = missdir(p_save)) == -1) {
				...
				fprintf(stderr,"Cannot chdir to <%s>: ", p_save);
				perror("");
				exit(2);
			}
		}
