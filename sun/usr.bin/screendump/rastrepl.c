#ifndef lint
static	char sccsid[] = "@(#)rastrepl.c 1.3 87/01/08 SMI";
#endif
	...
	char	**argv;
{
	...
	if ((output_pr = 
		mem_create(input_pr->pr_size.x * 2, input_pr->pr_size.y * 2,
			input_pr->pr_depth)) == NULL)
	...
	colormap_t *colormap;
{
	...
	if (!(from = malloc((unsigned) nb + nob)))
		error(ERR_NOMEM);
