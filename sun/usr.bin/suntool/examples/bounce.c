#ifndef lint
static char sccsid[] = "@(#)bounce.c 1.1 86/09/25 Copyr 1986 Sun Micro";
#endif
...
Restart:
	...
	pw_writebackground(pw, 0, 0, rect->r_width, rect->r_height,
			   PIX_SRC);
	/*
	 * my_done calls notify_dispatch to dispatch events to the frame
	 * regularly. it returns TRUE if it's time to finish.
	 * This will call my resize and repaint procs if necessary 
	 */
	while (gfx->gfx_reps) {
