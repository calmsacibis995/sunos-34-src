#ifndef lint
static	char sccsid[] = "@(#)input.c 1.1 86/09/25 Copyr 1984 Sun Micro";
#endif
...
struct timeval **timer;
{
	...
	*ibits = *obits = *ebits = 0;
    ...
    if (ie.ie_code <= ASCII_LAST) { /* input was from the keyboard */
	fted_scroll_char_pressed(ie.ie_code);
	*ibits = *obits = *ebits = 0;
	return;
    }
    if (*timer == NULL) {
	switch (ie.ie_code) {
	    ...
	    case MENU_BUT:
		    ...
		    menu_item = menu_display(&fted_menu_ptr, &ie,
				fted_canvas_sub_win->ts_windowfd);
		    if (menu_item != NULL) {
			panel_set_value(fted_draw_options_item, menu_item->mi_data);
		        fted_set_drawing_option(0, menu_item->mi_data, 0);
		    }
		}
		*ibits = *obits = *ebits = 0;
		return;
