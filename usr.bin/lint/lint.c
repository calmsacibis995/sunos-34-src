#ifndef lint
static	char sccsid[] = "@(#)lint.c 1.1 86/09/25 SMI"; /* from UCB 1.5 03/30/83 */
#endif
...
#ifndef FLEXNAMES
					if( flag ) werror( "%.8s evaluation order undefined", s);
#else
					if( flag ) werror( "%s evaluation order undefined", s);
#endif
					}
...
#ifndef FLEXNAMES
	if( s->sclass == REGISTER ) uerror( "can't take & of %.8s", s->sname );
#else
	if( s->sclass == REGISTER ) uerror( "can't take & of %s", s->sname );
#endif
	return(1);
	}
...
commdec( class, i ){
	...
	s = STP(i);
	if (s == NULL) return;
	if( class == STATIC ) outdef( s, LST, USUAL );
	else outdef( s, libflag?LIB:LDC, USUAL );
	}
