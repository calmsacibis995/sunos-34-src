#ifndef lint
static	char sccsid[] = "@(#)iostat.c 1.1 86/09/25 SMI"; /* from UCB 4.9 83/09/25 */
#endif
/*
 * iostat
 */
...
char *argv[];
{
	...
	lseek(mf, (long)nl[X_DK_BPS].n_value, 0);
	read(mf, s.dk_bps, sizeof s.dk_bps);
...
{
	register i;
	double atime, bytes, xtime, itime;
	...
	atime = s.dk_time[dn];
	atime /= hz;
	bytes = s.dk_wds[dn]*64.0;	/* number of bytes transferred */
	xtime = bytes/s.dk_bps[dn];	/* transfer time */
	itime = atime - xtime;		/* time not transferring */
/*
	printf("\ndn %d, bytes %8.2f, atime %6.2f, xtime %6.2f, itime %6.2f\n",
	    dn, bytes, atime, xtime, itime);
*/
	if (xtime < 0)
		itime += xtime, xtime = 0;
	if (itime < 0)
		xtime += itime, itime = 0;
	printf("%4.0f", bytes/1024/etime);	/* 1K blocks per second */
	printf("%4.0f", s.dk_xfer[dn]/etime);	/* transfers per second */
	printf("%5.1f ",
