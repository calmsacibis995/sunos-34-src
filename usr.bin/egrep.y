...
%{
static	char sccsid[] = "@(#)egrep.y 1.1 86/09/25 SMI"; /* from UCB 4.4 5/29/85 */
#include <stdio.h>
#include <ctype.h>
#include <sys/types.h>
#include <sys/stat.h>
...
char **argv;
{
	...
	while(( c = getopt(argc, argv, "shbcie:f:lnv")) != EOF)
		switch(c) {
		...
		found:	for(;;) {
				...
				if (--ccount <= 0) {
					if (p <= &buf[blksize]) {
						...
						ccount = read(f, p, &buf[2*blksize]-p);
					}
					if (ccount <= 0) {
						if (ccount < 0)
							goto readerr;
