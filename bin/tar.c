...
#ifndef lint
static	char sccsid[] = "@(#)tar.c 1.1 86/09/24 SMI"; /* from UCB 5.7 86/04/26 */
#endif not lint
...
char	*file;
{
	...
	fp = fopen(file, "r");
	if (fp == NULL) {
		fprintf(stderr, "tar: could not open exclude file %s: ",
			file);
		perror("");
		exit(1);
	}
...
char	*str;
{
	...
	exp->name = strcpy(malloc(strlen(name)), name);
