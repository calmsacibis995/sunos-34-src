...
#ifndef lint
static	char *sccsid = "@(#)sh.exec.c 1.1 86/09/24 SMI; from UCB 5.2 6/6/85";
#endif
...
#else notyet
	/*
	 * Have to do it this way until FIOCLEX has same semantics as
	 * in 4.3bsd.  Once that happens, we can get rid of closech().
	 */
	closech();
#endif notyet
	...
	register char **t;
{
	...
	case ENOEXEC:
		...
		{       
			...
			if (ff != -1 && read(ff, &ch, 1) == 1 && !isprint(ch)
			       && !isspace(ch)) {
