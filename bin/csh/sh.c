...
#ifndef lint
static	char *sccsid = "@(#)sh.c 1.1 86/09/24 SMI; from UCB 5.2 6/6/85";
#endif
	...
	char **av;
{
	...
	if (prompt) {
		char prompt[32+3];
		gethostname(prompt, sizeof (prompt) - 3);
		strcat(prompt, uid == 0 ? "# " : "% ");
		set("prompt", prompt);
	}
...
#ifdef notdef
	/*
	 * Can't do this without a 4.3 kernel: close on exec
	 * currently propagated to dup'ed descriptors, which
	 * has the effect of closing stdin, stdout, and stderr
	 * for all exec'ed commands.
	 */
