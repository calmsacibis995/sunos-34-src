#ifndef lint
static	char sccsid[] = "@(#)pr.c 1.1 86/09/24 SMI"; /* from S5R2 1.3 */
#endif
...
main(argc, argv) char *argv[];
{
	...
	if (Multi == 'm') {
		if (!nfdone) { /* no files named, use stdin */
			if (mustopen(NOFILE, &Files[Nfiles++]) == NULL)
				++noprint; /* suppress printing */
		}
		if (!noprint)
			print(NOFILE);	/* print all files */
	} else {
		if (!nfdone) /* no files named, use stdin */
