#! /bin/sh
#
#	@(#)lint.sh 1.1 86/09/24 SMI; from S5R2 1.5
#
...
LDIR=/usr/5lib/lint		# where first & second pass are
...
then
	...
	do
		...
		*.c)	T=`basename $i .c`.ln
			...
			(cc $CCF -I/usr/5include $i | $LDIR/lint1 $LINTF -H$HOUT $i >$T) 2>&1
			$LDIR/lint2 -H$HOUT
			rm -f $HOUT;;
		esac
	done
	...
	do
		...
		*.c)	if [ "$NDOTC" ]
			...
			(cc $CCF -I/usr/5include $i|$LDIR/lint1 $LINTF -H$HOUT $i >>$TOUT)2>&1;;
		esac
	done
	if [ "$LLIB" ]
	then
