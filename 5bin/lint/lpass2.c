#ifndef lint
static	char sccsid[] = "@(#)lpass2.c 1.1 86/09/24 SMI"; /* from S5R2 1.8 */
#endif
...
main( argc, argv ) char *argv[]; 
{
	...
	while((c=getopt(argc,argv,"abchnpuvxzH:LT:X:")) != EOF)
		switch(c) {
		case 'h':
			hflag = 0;
			break;
...
#endif
	if ( ( cfno = ffree++ ) >= FSZ )
		lerror( "Too many file names", CCLOSE | FATAL | ERRMSG );
}
...
chktype( pt1, pt2 ) register ATYPE *pt1, *pt2; 
{
	...
	if( (t=BTYPE(pt1->aty))==STRTY || t==UNIONTY || t==ENUMTY )
