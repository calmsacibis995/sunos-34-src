#ifndef lint
static	char sccsid[] = "@(#)pftn.c 1.1 86/09/24 SMI"; /* from S5R2 1.2 */
#endif
...
psave( i )
{
	...
	if(paramstk == NULL) {
	    ...
	    paramstk = (int*)realloc(paramstk, (paramno+PARAMSZ)*sizeof(int));
	}
	if (paramstk == NULL)
...
ftnend(){ /* end of function */
	...
	swx = 0;
	reset_swtab();
	if (paramstk != NULL) free(paramstk);
	paramstk = NULL;
	paramno = 0;
	locctr(DATA);
	}
...
# endif
		...
		if( ISARY(t) ){
			...
			id = dimtab[pstk->in_x];
			p = STP(id);
			if( pstk->in_x == -1 || p==NULL || p->sclass != MOS && !(p->sclass&FIELD) ) fatal("initialization with non-exist structure" );
			t = p->stype;
			d = p->dimoff;
			s = p->sizoff;
			off += p->offset;
...
fixclass( class, type ) TWORD type; {
	...
	case REGISTER:
		...
		if( blevel == 0 ) UERROR( MESSAGE( 68 ), curname );
	...
	case ULABEL:
		/* "illegal class for %s" */
		if( blevel < 2 ) UERROR( MESSAGE( 52 ), curname );
		return( class );

	case PARAM:
		/* "illegal class for %s" */
		if( blevel != 1 ) UERROR( MESSAGE( 52 ), curname );
		return( class );
...
#else
		if (ddebug)
		    printf("removing %.8s from [ %d], flags %o level %d\n",
			p->sname,p,p->sflags,p->slevel);
#endif
# endif
		if( p->sflags & SHIDES )
...
hide( p ) register struct symtab *p; {
	...
	*q = *p;
	...
	if (blevel==2 && p->slevel==1) WERROR( MESSAGE( 131 ), p->sname );
# ifndef BUG1
