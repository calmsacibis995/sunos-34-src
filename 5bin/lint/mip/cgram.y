%{
#ifndef lint
static	char sccsid[] = "@(#)cgram.y 1.1 86/09/24 SMI"; /* from S5R2 1.7 */
#endif
%}
...
switchpart:	   SWITCH  LP  e  RP
			={	int type;
				...
				if( type == FLOAT || type == DOUBLE || type == STRTY || type == UNIONTY )
...
more_swtab()
{
	...
	if(swtab == NULL) {
	    ...
	    swtab = (struct sw*)realloc(swtab, (i+SWITSZ)*sizeof(struct sw));
	}
	swp = swtab + i;
    }
}
...
setdim() { /*  store dimtab info on block entry */
	...
	if (dimptr >= &dimrec[BNEST]) UERROR( MESSAGE( 129 ) );
	dimptr->index  = curdim;
	dimptr->cextern = 0;
	}
