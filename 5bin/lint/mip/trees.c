#ifndef lint
static	char sccsid[] = "@(#)trees.c 1.1 86/09/24 SMI"; /* from S5R2 1.4 */
#endif
...
# endif
	...
	if( opty == UTYPE && l->in.op == ICON ){
		...
		case UNARY MINUS:
		case COMPL:
			if( !ISPTR(l->in.type) && conval( l, o, l ) ) return(l);
			break;
	...
	else if (o == CAST && r->in.op == ICON)
	{
		...
		case LS:
		case RS:
			if( !ISPTR(l->in.type) && !ISPTR(r->in.type) && conval( l, o, r ) ) {
				r->in.op = FREE;
				return(l);
				}
			break;
...
makety( p, t, d, s ) register NODE *p; TWORD t; {
	...
	if( t & TMASK ){
		/* non-simple type */
# if defined(EARLYOPT) || defined(LINT)
...
#ifdef FLEXNAMES
			p->in.name = tstr(exname(sp->sname));
#else
			register char *cp;
			register i;
			cp = exname( sp->sname );
