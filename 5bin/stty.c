#ifndef lint
static	char sccsid[] = "@(#)stty.c 1.1 86/09/24 SMI"; /* from S5R2 1.6 */
#endif
...
prmodes()
{
	...
	if (term == SYNC) {
		m = stio.imode;
		if (m & IUCLC) printf ("iuclc ");
		else printf ("-iuclc ");
		m = stio.omode;
		if (m & OLCUC) printf ("olcuc ");
		else printf ("-olcuc ");
		if (m & TAB3) printf ("tab3 ");
		m = stio.lmode;
		if (m & XCASE) printf ("xcase ");
		else printf ("-xcase ");
		if (m & STFLUSH) printf ("stflush ");
		else printf ("-stflush ");
		if (m & STWRAP) printf ("stwrap ");
		else printf ("-stwrap ");
		if (m & STAPPL) printf ("stappl ");
		else printf ("-stappl ");
		printf ("\n");
	}
	if (term == ASYNC) {
...
#if 0
	if(term == SYNC) {
		printf("-stflush "+((m&STFLUSH)!=0));
		printf("-stwrap "+((m&STWRAP)!=0));
		printf("-stappl "+((m&STAPPL)!=0));
	}
#endif
	printf("\n");
	m = cb.c_oflag;
