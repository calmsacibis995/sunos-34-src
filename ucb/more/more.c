#ifndef lint
static  char sccsid[] = "@(#)more.c 1.1 86/09/25 SMI"; /* from UCB 4.16 83/08/26 */
#endif
...
char *argv[];
{
    ...
    /* allow clreol only if Home and eraseln strings are
     *  defined, and in that case, make sure we are in noscroll mode
     */
    if(clreol) {
		if (Home == 0 || *Home == '\0' || eraseln == 0 || *eraseln == '\0') {
			clreol = 0;
		} else  {
			noscroll = 1;
		}
...
initterm ()
{
	...
	if ((term = getenv("TERM")) == 0 || tgetent(buf, term) <= 0) {
		...
		if (EodClr == NULL || *EodClr == 0) {
			/* terminal is missing clear-to-end-of-display feature */
			EodClr = eraseln; /* do the best we can */
		}
	}
