...
#ifndef lint
static char sccsid[] = "@(#)vfontedpr.c 1.1 86/09/25 SMI"; /* from UCB 5.1 (Berkeley) 6/5/85 */
#endif not lint
...
#define isidchr(c) (isalnum(c) || ((c) != NIL && index(l_idchars, (c)) != NIL))
    ...
    boolean	force;		/* true if we should force nokeyw */
{
	...
	if (*start == '\t') {
	    while (*start == '\t')
		start++;
	    i = tabs(_start, start) - margin / tabsize;
	    printf ("\\h'|%dn'",
