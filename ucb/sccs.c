#ifndef lint
static	char sccsid[] = "@(#)sccs.c 1.1 86/09/25 SMI"; /* from UCB 1.71 07/02/83 */
#endif
...
struct sccsprog SccsProg[] =
{
	...
	"-diff",	DODIFF,	NO_SDOT|REALUSER,	"diff",
...
# ifndef V6
		execvp(cmd->sccspath, ap);
# else
		execv(cmd->sccspath, ap);
# endif
	...
	char *gfile;
{
	...
	else
	{
		...
		command(&getv[1], FALSE, "-diff:elsfnhbwitC");
	}
	return (rval);
}

