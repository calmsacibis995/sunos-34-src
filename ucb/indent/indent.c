...
#ifndef lint
static char sccsid[] = "@(#)indent.c 1.4 86/12/24 Copyright 1985 Sun Micro";	/* from UCB 4.3 */
#endif not lint
    ...
    char      **argv;
{
	...
	while (ps.search_brace) {	/* if we scanned an if(), while(),
	    ...
	    case comment:	/* we have a comment, so we must copy it
				 * into the buffer */
		if (!flushed_nl) {
		    if (sc_end == 0) {	/* if this is the first comment,
		...
		if (flushed_nl)	/* if we flushed a newline, make sure it
				 * is put back */
		    force_nl = true;
		...
		if (force_nl) {	/* if we should insert a nl here, put it
				 * into the buffer */
		    force_nl = false;
		    --line_no;	/* this will be re-increased when the nl
...
check_type:
	...
	if (
		...
		*e_code = '\0';	/* null terminate code sect */
		...
		e_com = s_com;
					 ...
					 * comment */
	    force_nl = false;	/* cancel forced newline after newline,
				 * form feed, etc */
	...
	case colon:		/* got a ':' */
		...
		*e_code++ = ':';
	    ...
	    ps.in_stmt = false;	/* seeing a label does not imply we are in
				 * a stmt */
	...
	case lbrace:		/* got a '{' */
	    ...
	    if (!ps.block_init)
		force_nl = true;/* force other stuff on same line as '{'
				 * onto new line */
	    else if (ps.block_init_level<=0) ps.block_init_level = 1;
	    else ps.block_init_level++;
	    ...
	    if (s_code == e_code)
		ps.ind_stmt = false;	/* dont put extra indentation on
					 * line with '{' */
...
/* ?		dec_ind = 0; */
	...
	case rbrace:		/* got a '}' */
	    if (ps.p_stack[ps.tos] == decl && !ps.block_init)/* semicolons can be omitted in
					      declarations */
		parse(semicolon);
	...
	case ident:		/* got an identifier or constant */
	    if (ps.in_decl) {	/* if we are in a declaration, we must
				 * indent identifier */
		if (ps.want_blank)
		    *e_code++ = ' ';
		ps.want_blank = false;
