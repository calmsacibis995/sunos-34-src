...
#ifndef lint
static	char *sccsid = "%Z%%M% %I% %E% SMI"; /* from UCB 7.11 6/7/85 */
#endif not lint
	...
	int c;
{
	...
	case S_IFREG:
		i = read(io, (char *) &exec.a_magic, sizeof(exec.a_magic));
		lseek(io, 0l, 0);
		if (i != sizeof(exec.a_magic))
			break;
		switch (exec.a_magic) {
		...
		default:
			...
			cp = (char *)&exec.a_magic;
			i = sizeof(exec.a_magic);
			while (i--) {
				if (*cp++ & ~0177) {
