#ifndef lint
static	char *sccsid = "@(#)names.c 1.1 86/09/25 SMI"; /* from S5R2 1.1 */
#endif
	...
	while (np != NIL) {
		...
		if (image < 0) {
			...
			if (image < 0) {
				...
				while (fgets(line, LINESIZE, fo) != NULL) {
...
quote:
		...
		if (!metoo){
			if (allnet) {
				strcpy(namebuf, gp->ge_name);
				cp = striphosts(namebuf);
			} else
				cp=gp->ge_name;
			/*
