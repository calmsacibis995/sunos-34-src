#ifndef lint
static	char sccsid[] = "@(#)tset.c 1.1 86/09/25 SMI"; /* from UCB 1.5 06/07/83 */
#endif
#
...
**			to CERASE if nothing else is specified.
...
**		CONTROL('U') -- control-U, the default for -k.
...
**
**	Requires:
**		Routines to handle htmp, ttytype, and termcap.
**
**	Compilation Flags:
...
# define	YES		1
# define	NO		0
# define	CONTROL(x)	(x ^ 0100)
# define	BACKSPACE	(CONTROL('H'))
...
#  ifdef UCB_NTTY
		...
		if (ldisc == NTTYDISC)
		{
			...
			ltc.t_suspc  = reset(ltc.t_suspc, CSUSP);
			ltc.t_dsuspc = reset(ltc.t_dsuspc, CDSUSP);
			ltc.t_rprntc = reset(ltc.t_rprntc, CRPRNT);
			ltc.t_flushc = reset(ltc.t_flushc, CFLUSH);
			ltc.t_werasc = reset(ltc.t_werasc, CWERASE);
			ltc.t_lnextc = reset(ltc.t_lnextc, CLNEXT);
...
#   ifdef TIOCGETC
		...
		tbuf.t_intrc = reset(tbuf.t_intrc, CINTR);   /* was ctrl(?) */
		tbuf.t_quitc = reset(tbuf.t_quitc, CQUIT);
		tbuf.t_startc = reset(tbuf.t_startc, CSTART);
		tbuf.t_stopc = reset(tbuf.t_stopc, CSTOP);
		tbuf.t_eofc = reset(tbuf.t_eofc, CEOF);
		/* brkc is left alone */
...
#  else USG
		...
		mode.c_cc[VINTR] = CHK(mode.c_cc[VINTR], CONTROL('?'));
		mode.c_cc[VQUIT] = CHK(mode.c_cc[VQUIT], CONTROL('\\'));
		mode.c_cc[VEOF] = CHK(mode.c_cc[VEOF], CONTROL('D'));
