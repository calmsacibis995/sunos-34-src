#ifndef lint
static	char sccsid[] = "@(#)gprof.c 1.1 86/09/25 SMI"; /* from UCB 1.11 5/11/83 */
#endif
    ...
    struct nlist	*nlistp;
{
	...
	/*
	 *	look for and ignore 'funny' names. 'Funny' includes:
	 *		.o file names 
	 *		(Others to be added.)
	 */
    for ( name = strtab + nlistp -> n_un.n_strx ; *name ; name += 1 ) {
	if ( *name == '.' && name[1] == 'o' && name[2] == '\0' ) {
	    return FALSE;
	}
    }
    return TRUE;
