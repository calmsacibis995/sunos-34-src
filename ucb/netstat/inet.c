#ifndef lint
static	char sccsid[] = "@(#)inet.c 1.1 86/09/25 SMI"; /* from UCB 4.14 83/09/21 */
#endif
	...
	char *proto;
{
	...
	if (strcmp(proto, "tcp") == 0)
		...
		return NULL;
	if (ntohs(port) >= 0 && ntohs(port) < MAXPORT
	    && names[protonum][ntohs(port)])
		return (names[protonum][ntohs(port)]);
	else
		return NULL;
}
...
initarray()
{
	...
	while(sv = getservent()) {
		if (strcmp(sv->s_proto, "tcp") == 0)
			...
			continue;
		if (ntohs(sv->s_port) < 0 || ntohs(sv->s_port) >= MAXPORT)
			continue;
		names[proto][ntohs(sv->s_port)] =
		    (char *)malloc(strlen(sv->s_name) + 1);
		strcpy(names[proto][ntohs(sv->s_port)], sv->s_name);
	}
	endservent();
}
