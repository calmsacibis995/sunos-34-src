#ifndef lint
static	char sccsid[] = "@(#)m4.c 1.1 87/01/08 SMI"; /* from UCB 1.2 12/16/82 */
#endif
#include <stdio.h>
#include <signal.h>
#ifdef SUNPRO
		...
		*tp = EOS;
		if (t<=0) {
			...
			if (**argv=='-')
				infile[infptr] = stdin;
#ifdef SUNPRO
			else if (((fd= open_vroot(argv[0], O_RDONLY, 0, NULL, VROOT_DEFAULT)) < 0) ||
				 (infile[infptr]=fdopen(fd, READ))==ERROR) {
#else
...
#ifdef SUNPRO
		fd= open_vroot(ap[1], O_RDONLY, 0, NULL, VROOT_DEFAULT);
		get_vroot_path(NULL, &name, NULL);
		(void)strcpy(name_copy, name);
		if ((fd < 0) || (infile[infptr] = fdopen(fd, READ))==ERROR) {
#else
