#ifndef lint
static	char sccsid[] = "@(#)cpp.c 1.1 87/01/08 SMI"; /* from S5R2 1.11 */
#endif
...
{
	if ( pflag == 0 )
		fprintf( fout, LINEFORM, lineno[ifno], fnames[ifno], incl );
}
	...
	char *p;
{
	...
	*/
	...
	/*
	 * "ex_blank" was set to 1 here previously; this was so that the
	 * blank that separated the macro name (and optional formals)
	 * from the macro body would not be included in the macro body.
	 * Unfortunately, blanks are significant in the pre-S5 C compiler
	 * (since "a=*b" means "a = a*b" while "a= *b" means "a = *b"),
	 * so we have to include the blanks in the definition so old code
	 * doesn't break.
	 */
	ex_blank = 0;
...
#	    endif
	...
	/*
	 * Scan -D and -U from command line into symbol table.
	 */
	tf = fnames[ifno];
