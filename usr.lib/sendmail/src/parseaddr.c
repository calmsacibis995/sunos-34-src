...
SCCSID(@(#)parseaddr.c 1.1 86/09/25 SMI); /* from UCB 4.15 4/28/85 */
...
# endif DEBUG
	...
	if ((senderaddress ? m->m_os_rwset : m->m_or_rwset) < 0)
		return (name);
	...
	*/
	...
	if (senderaddress)
	{
		rewrite(pvp, 1);
		if (m->m_os_rwset > 0)
			rewrite(pvp, m->m_os_rwset);
	}
	else
	{
		rewrite(pvp, 2);
		if (m->m_or_rwset > 0)
			rewrite(pvp, m->m_or_rwset);
	}
