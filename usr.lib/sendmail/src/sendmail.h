...
# define EXTERN
# ifndef lint
static char SmailSccsId[] =	"@(#)sendmail.h 1.1 86/09/25 SMI"; /* from UCB 4.13 4/4/85 */
# endif lint
# else  _DEFINE
...
struct mailer
{
	...
	short	m_os_rwset;	/* rewriting set for outgoing senders */
	short	m_or_rwset;	/* rewriting set for outgoing recipients */
	short	m_is_rwset;	/* rewriting set for incoming senders */
	short	m_ir_rwset;	/* rewriting set for incoming recipients */
...
# define WKPRIFACT	200		/* bytes each pri point is worth */
...
/*
**  Information about hosts which we have looked up in the host
**  databases and/or have connected to.
*/
struct hostinfo
{
	struct in_addr	h_addr;		/* host Internet address */
...
struct symtab
{
	...
	union
	{
		...
		char	*sv_alias;	/* alias */
		struct hostinfo sv_host;	/* host system info */
	}	s_value;
};
...
EXTERN char	*PostmasterCopy;/* addrs to cc on returntosender() mail */
