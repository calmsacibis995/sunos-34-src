...
SCCSID(@(#)collect.c 1.1 86/09/25 SMI); /* from UCB 4.5 4/28/85 */
...
**
**	Parameters:
**		fromnet -- if set, give an ARPANET style message
**			to say we are ready to collect input, and
**			time out after ~30 minutes of inactivity.
**			(Actual timeout set in config file.)
**
**	Returns:
**		none.
**
	...
	bool fromnet;
{
	...
	register char *p;
	extern char *hvalue();
	time_t timeout = fromnet? ReadTimeout : (time_t)0;
	...
	*/
	...
	(void) sfgets(buf, sizeof buf, InChannel, timeout);
	fixcrlf(buf, FALSE);
# ifndef NOTUNIX
	...
	*/
	...
	do
	{
		...
		while ((c = getc(InChannel)) == ' ' || c == '\t')
		{
			...
			*p++ = '\n';
			*p++ = c;
			if (sfgets(p, MAXFIELD - (p - buf), InChannel, timeout)
			    == NULL)
				break;
			fixcrlf(p, TRUE);
		}
		...
		*/
		...
		if (bitset(H_EOH, chompheader(buf, FALSE)))
			break;
	} while (sfgets(buf, MAXFIELD, InChannel, timeout) != NULL);
...
# endif DEBUG
	...
	if (buf[0] == '\0')
		(void) sfgets(buf, MAXFIELD, InChannel, timeout);
...
# endif DEBUG
			/* "apparently-to" is ugly, make it a real "to" */
			addheader("to", q->q_paddr, CurEnv);
		}
	}
