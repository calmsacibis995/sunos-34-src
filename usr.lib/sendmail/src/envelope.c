...
SCCSID(@(#)envelope.c 1.1 86/09/25 SMI); /* from UCB 4.9 4/20/85 */
	...
	register ENVELOPE *e;
{
	...
	if (e == CurEnv)
		e->e_from = NullAddress;
	else
		e->e_from = CurEnv->e_from;
	e->e_parent = parent;
	e->e_ctime = curtime();
...
# endif DEBUG
	...
	{
		if (from) syslog(LOG_ERR, "Unparseable user %s wants to be %s",
				realname, from);
		from = newstr(realname);
		if (parseaddr(from, &CurEnv->e_from, 1, '\0') == NULL)
		   (void)parseaddr("root", &CurEnv->e_from, 1, '\0');
	}
	else
		FromFlag = TRUE;
