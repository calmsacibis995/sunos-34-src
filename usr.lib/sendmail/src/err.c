...
SCCSID(@(#)err.c 1.1 86/09/25 SMI); /* from UCB 4.6 4/4/85 */
...
# ifdef LOG
	if (LogLevel > 0)
		syslog(LOG_NOTICE, "%s: SYSERR: %s", CurEnv->e_id, &MsgBuf[4]);
# endif LOG
	errno = 0;
	if (QuickAbort)
		longjmp(TopFrame, 2);
...
**		num -- the default ARPANET error number (in ascii)
**		msg -- the message (printf fmt) -- if it begins
**			with 3 digits, this number overrides num.
**		a, b, c, d, e -- printf arguments
**
**	Returns:
**		none
...
**
**	Parameters:
**		err_no -- the error number (eg from errno) to translate
**
**	Returns:
	...
	int err_no;
{
	...
	if (err_no == ETIMEDOUT || err_no == ECONNRESET) {
		  strcpy(buf, sys_errlist[err_no]);
		  if (SmtpPhase) {
		      strcat(buf," during ");
		      strcat(buf,SmtpPhase);
		  }
		  if (RealHostName) {
		      strcat(buf," with ");
		      strcat(buf,RealHostName);
		  }
		  return(buf);
		}

	if (err_no == ENAMESER) {
		(void) sprintf(buf,"Name server error for %s", RealHostName);
		return(buf);
	}
	if (err_no == EHOSTDOWN && RealHostName != NULL) {
		(void) sprintf(buf,"Host %s is down", RealHostName);
		return(buf);
	}

	if (err_no == ECONNREFUSED && RealHostName != NULL) {
	...
	(void) sprintf(buf, "Error %d", err_no);
	return (buf);
}
