#ifndef lint
static	char sccsid[] = "@(#)termcap.c 1.1 86/09/25 SMI"; /* from UCB 4.1 6/27/83 */
#endif
	...
	register char *bp;
{
	...
	if (ioctl(1, TIOCGSIZE, (char *)&ts) < 0)
		return;
	if (ts.ts_lines == 0 || ts.ts_cols == 0 ||
	    ts.ts_lines > 999 || ts.ts_cols > 999)
		return;
	...
	*bp++ = 'i';
	*bp++ = '#';
	bp = appendsmalldec(bp, ts.ts_lines);
	*bp++ = ':';
	*bp++ = 'c';
	*bp++ = 'o';
	*bp++ = '#';
	bp = appendsmalldec(bp, ts.ts_cols);
	*bp++ = ':';
