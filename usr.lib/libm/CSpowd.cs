#ifdef sccsid
static  char sccsid[] = "@(#)CSpowd.cs 1.1 86/09/25 Copyr 1986 Sun Micro";
#endif
...
double S_pow_core( x, y ) /* computes exp(y * log(x)) */ 
double x, y ;
{
double CSexpd() ; double CSlogd () ; double CFsqrtd () ;
...
if (dclass(y) == zero) 
	...
	if (iys == 0) switch (dclass(x))
		...
		case qnan: 
		case inf:
		case zero: return(CSexpd(y0*CSlogd(x))) ;
		}
	expfactor = 1.0 ; 
	}
	else
...
switch (dclass(x))
	...
	sigx = FFscaled_( &x, &expx ) ; /* sqrt(0.5) <= sigx <= sqrt(2.0) */
	expx = -expx ;
	}
ny = expx * y ;
switch (dclass(ny))
	...
	case snan:
	case qnan:
	case inf:	return(CSexpd(y0*CSlogd(x))) ;
...
expfactor =  CSexpd( fracny * 0.69314718055994530941 + y * CSlogd(sigx) ) ;
expfactor = FFscaled_( &expfactor, &inty ) ;
}
...
double arg1, arg2;
{
	...
	if(dminus(arg1) != 0) 
		...
		switch (l)
			{
			case 1:return(-S_pow_core(-arg1,arg2)) ;
			case 2:return( S_pow_core(-arg1,arg2)) ;
			}
		}
	return( S_pow_core( arg1,arg2)) ;
}

double S_pow_fast( x, y ) /* computes exp(y * log(x)) for single precision */ 
double x, y ;
{
return( CSexpd( y * CSlogd(x) ) ) ;
}
