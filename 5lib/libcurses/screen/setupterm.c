#ifndef lint
static	char sccsid[] = "@(#)setupterm.c 1.4 86/12/13 SMI"; /* from S5R2 1.1 */
#endif

#include "curses.ext"
#include "../local/uparm.h"
#include <sys/ttold.h>		/* so the TIOCGSIZE "ioctl" is picked up */
...
{
	char tiebuf[4096];
	char fname[1024];	/* MAXPATHLEN, sigh */
	register char *ip;
	register char *cp;
	int n, tfd;
	char *lcp, *ccp;
	...
	if ((cp=getenv("TERMINFO")) && *cp) {
		if (strlen(cp) + 2 + strlen(term) >= 1024) {
			if (errret == 0) {
...
corrupt:
	 ...
	 */
	{
		...
		if (ioctl(cur_term->Filedes, TIOCGSIZE, &w) != -1 &&
