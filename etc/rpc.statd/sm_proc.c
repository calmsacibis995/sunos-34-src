#ifndef lint
static char sccsid[] = "@(#)sm_proc.c 1.1 86/09/24 Copyr 1984 Sun Micro";
#endif
...
sm_name *namep;
{
	...
	if(debug) printf("proc sm_stat: mon_name = %s\n", namep);
...
mon *monp;
{
	...
	monidp = &monp->mon_id;
/*
	if(debug) printf("proc sm_mon: mon_name = %s, id = %d\n", 
...
/*
	if(debug) printf("proc sm_unmon: mon_name = %s, [%s, %d, %d, %d]\n",
...
my_id *myidp;
{
	...
	if(debug) printf("proc sm_unmon_all: [%s, %d, %d, %d]\n",
		myidp->my_name,
...
sm_simu_crash_1()
{
	if(debug) printf("proc sm_simu_crash\n");
/*
	if(monitor_q != NULL)
*/
	sm_crash();
...
mon *monp;
{
	...
	if(debug) printf("add_mon(%x) %s (id=%d)\n",
	...
	while((nl = next) != NULL) {
		...
		if(mon_name == NULL || (mon_name != NULL &&
	 	strcmp(nl ->id.mon_id.mon_name, mon_name) == 0)) {
			...
			my_idp->my_proc == nl_idp->my_proc) {
				/* found */
				if(debug) printf("delete_mon(%x): %s\n", nl, mon_name);
				if(nl->prev != NULL)
					nl->prev->nxt = nl->nxt;
				else {
					monitor_q = nl->nxt;
...
/*
				if(debug) printf("prblem with notifying %s failure, give up\n", mon_name);
*/
			}
		}
		next = next->nxt;
...
int state;
{
	...
	if(debug) printf("statd_call_lockd: %s state = %d\n",
	...
	if((err = call_tcp(my_idp->my_name, my_idp->my_prog, my_idp->my_vers,
		...
		fprintf(stderr, "statd cannot contact local lockd on %s status change, give up!\n", mon_name);
		return(-1);
		}
	else
		return(0);
