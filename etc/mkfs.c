...
#ifndef lint
static	char *sccsid = "@(#)mkfs.c 1.1 86/09/24 SMI"; /* from UCB 5.3 9/11/85 */
#endif not lint
 ...
 * usage:
 *    mkfs -N special size
 *	[ nsect ntrak bsize fsize cpg minfree rps nbpi opt apc ]
 */
	...
	char *argv[];
{
	...
	if (argc < 2) {
		...
		printf("[ nsect ntrak bsize fsize cpg minfree rps nbpi opt ]\n");
		exit(1);
	}
	fsys = argv[0];
	fssize = atoi(argv[1]);
