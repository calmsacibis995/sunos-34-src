#ifndef lint
static char sccsid[] = "@(#)prot_lock.c 1.1 86/09/24 Copyr 1984 Sun Micro";
#endif
...
struct fs_rlck	*grant_q;		/* ptr to grant queue*/
struct fs_rlck	*monitor_q;		/* ptr to monitor queue*/
...
reclock *a;
{
	...
	if((fp = get_fe()) == NULL) {	/*no more fe entry*/
		...
		if((fp->svr = xmalloc(strlen(a->lck.svr))) == NULL) {
			free_fe(fp);
			return(NULL);
		}
		strcpy(fp->svr, a->lck.svr);
...
reclock *b;
{
	...
	if(( a= get_le()) == NULL)
		return(NULL);
	if((a->lck.svr = xmalloc(strlen(b->lck.svr))) == NULL) {
		free_le(a);
		return(NULL);
	}
	strcpy(a->lck.svr, b->lck.svr);
	...
	if((a->lck.clnt = xmalloc(strlen(b->lck.clnt))) == NULL) {
		free_le(a);
		return(NULL);
	}
	strcpy(a->lck.clnt, b->lck.clnt);
	...
	if(i == 1) { /* insert! */
		if((new = find_me(site, proc)) == NULL) { /* not found*/
			if(( new = get_me()) == NULL) /* no more me entry */
				...
				if((new->svr = xmalloc(strlen(site))) == NULL) {
					used_me--;
					free((char *) new);
					return(-1);
				}

				strcpy((char *) new->svr, site);
...
int i;
{
	...
	case 1:
		...
		break;
	default:
		fprintf("unknown contact monitor (%d)\n", i);
		abort();
	}
	...
	if(resp->res_stat == stat_succ) {
		if(resp->sm_stat == stat_succ) {
			...
			fprintf("site %s not subscribe to status monitor service \n", new->svr);
			return(-1);
		}
	}
	else {
