...
#ifndef lint
static	char sccsid[] = "@(#)in.rlogind.c 1.1 86/09/24 SMI"; /* from UCB 5.11 5/23/86 */
#endif
...
# ifdef sun
	 /*
	  * be careful of byte alignment on non-vaxen
	  */
	ts.ts_lines = (cp[4] << 8) + cp[5];
	ts.ts_cols = (cp[6] << 8) + cp[7];
	(void)ioctl(pty, TIOCSSIZE, &ts);
# else sun
