...
#ifndef lint
static	char sccsid[] = "@(#)inet.c 1.3 86/12/12 SMI"; /* from UCB 5.4 6/3/86 */
#endif not lint
...
/*
 * like inet_netmatch, but don't check the subnet bits
*/
inet_netonly(s1, s2)
	struct sockaddr_in *s1, *s2;
{
	register u_long i1 = ntohl(s1->sin_addr.s_addr);
	register u_long i2 = ntohl(s2->sin_addr.s_addr);

	if (IN_CLASSA(i1))
		return( (i1 & IN_CLASSA_NET) == (i2 & IN_CLASSA_NET));
	else if (IN_CLASSB(i1))
		return( (i1 & IN_CLASSB_NET) == (i2 & IN_CLASSB_NET));
	else
		return( (i1 & IN_CLASSC_NET) == (i2 & IN_CLASSC_NET));
}
...
printf("    r=%x, d=%x, same, state=%d\n",r,d,rt->rt_state & RTS_INTERNAL );
