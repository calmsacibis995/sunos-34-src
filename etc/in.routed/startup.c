...
#ifndef lint
static	char sccsid[] = "@(#)startup.c 1.4 87/01/05 SMI"; /* from UCB 5.3 2/7/86 */
#endif not lint
...
ifinit()
{
	...
	lookforinterfaces = 0;
		...
		if ((ifs.int_flags & IFF_UP) == 0 ||
			...
			if (ifp = if_ifwithaddr(&ifs.int_addr))
				if_purge(ifp);
			lookforinterfaces = 1;
			continue;
		}
	...
	struct interface *ifp;
{
	...
	struct sockaddr *dst;
	int state = ifp->int_flags & (IFF_INTERFACE|IFF_PASSIVE|IFF_REMOTE);
	struct rt_entry *rt;

	if (ifp->int_flags & IFF_POINTOPOINT) {
		if (ifp->int_metric == 0) {	/* local side of pt-pt */
			dst = &ifp->int_addr;
			rt = rtlookup(dst);
			if (rt == 0 || !equal(&ifp->int_addr, &rt->rt_router)) {
				rtadd(dst, dst, 0, state);
				if (rt)
					rtdelete(rt);
			}
		}
...
#define	readentry(fp) \
	...
	for (;;) {
		...
		if (strcmp(qual, "passive") == 0) {
			...
			if (ioctl(s, SIOCADDRT, (char *)&route.rt_rt) < 0)
			  perror("passive add");
			continue;
		}
