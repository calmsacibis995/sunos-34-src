#ifndef lint
static	char *sccsid = "@(#)kgmon.c 1.1 86/09/25 SMI"; /* from UCB 4.8 83/07/01 */
#endif
	...
	char *argv[];
{
	...
	if (kflag) {
		...
		off = nl[N_SYSMAP].n_value - KERNELBASE;
		lseek(kmem, off, 0);
		nl[N_SYSSIZE].n_value *= sizeof (struct pte);
	...
	int fd, base, off;
{
	...
	if (kflag) {
		/* get kernel pte */
		base = ((int)ptob(Sysmap[btop(base - KERNELBASE)].pg_pfnum)) +
			(base & PGOFSET);
	}
