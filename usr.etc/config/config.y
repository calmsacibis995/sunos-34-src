...
#ifndef lint
static	char sccsid[] = "@(#)config.y 1.1 86/09/25 SMI"; /* from UCB 1.18 83/05/18 */
#endif
...
Config_spec:
	...
	System_spec
		|
	HZ NUMBER
	      = { yywarn("HZ specification obsolete; delete"); } |
	TIMEZONE NUMBER
	  ...
	  ARGS optional_on arg_device_spec
		= {
			...
			if (fl && fl->f_argdev != NODEV)
				yywarn("extraneous arg device specification");
			else
				fl->f_argdev = $3;
		}
	;
...
check_tz()
{
	...
	if (abs(timezone) > 12 * 60)
		yywarn("timezone is unreasonable");
	else
		hadtz = 1;
}
...
"Warning, swap defaulted to 'b' partition with root on '%c' partition",
	...
	if (fl->f_argdev != swap->f_swapdev)
		yywarn("Warning, arg device different than primary swap");
	/*
	...
	register dev_t *pchecked;
{
	...
	for (;fl && fl->f_type == SWAPSPEC; fl = fl->f_next) {
		...
		if (!finddev(fl->f_swapdev)) {
			(void)sprintf(errbuf, "config: swap device %s not configured",
			   fl->f_fn);
			yyerror(errbuf);
		}
	...
	char *systemname, *devtype;
{
	...
	(void)sprintf(errbuf, 
	    "config: %s: %s device not configured", systemname, devtype);
	yyerror(errbuf);
}

/*
 * Check to see if there is GENERIC specified somewhere
 * as a generic swap was specified.  If none is found,
...
checkgeneric()
{
	...
	if (eq(raise(ident), "GENERIC"))
		return;
	for (cp = cputype; cp; cp = cp->cpu_next)
		if (eq(cp->cpu_name, "GENERIC"))
			return;
	for (op = opt; op; op = op->op_next)
		if (eq(op->op_name, "GENERIC"))
			return;

	op = (struct opt *)malloc(sizeof (struct opt));
	op->op_name = ns("GENERIC");
	op->op_next = opt;
	op->op_value = 0;
	opt = op;
}
...
struct bus_info sun3_info[] = {
	{ "virtual",	SP_VIRTUAL,	(1<<32)-1 },
	{ "obmem",	SP_OBMEM,	(1<<32)-1 },
	{ "obio",	SP_OBIO,	(1<<21)-1 },
	{ "vme16d16",	SP_VME16D16,	(1<<16)-1 },
	{ "vme24d16",	SP_VME24D16,	(1<<24)-(1<<16)-1 },
	{ "vme32d16",	SP_VME32D16,	(1<<32)-(1<<24)-1 },
	{ "vme16d32",	SP_VME16D32,	(1<<16) },
	{ "vme24d32",	SP_VME24D32,	(1<<24)-(1<<16)-1 },
	{ "vme32d32",	SP_VME32D32,	(1<<32)-(1<<24)-1 },
	{ (char *)0,	0,	0 }
};
