#ifndef lint
static char sccsid[] = "@(#)ns_req.c 1.1 86/09/25 SMI"; /* from UCB 3.22 4/1/86 */
#endif
	...
	struct sockaddr_in *from;
{
	...
	char **dpp;
	char *dnp;
	int n, class, type, count, foundname, founddata;
...
#ifdef DEBUG
		if (debug >= 5)
		    fprintf(ddt,"foundname = %d, count = %d, founddata = %d\n",
			foundname, count, founddata);
#endif
		if (count) {
	...
	findns:
		...
		foundns:
			...
			if (nspp != nsp) {
				*nspp = NULL;
				if ((n =
				    ns_forw(nsp, msg, msglen, from, qsp)) == 0)
					return;
				if (n == -2) {
					hp->rcode = SERVFAIL;
					break;
	...
	int msglen;
{
	...
	if (write(rfd, (char *)&len, sizeof(len)) != sizeof(len) ||
	    write(rfd, msg, msglen) != msglen) {
#ifdef DEBUG
		if (debug)
