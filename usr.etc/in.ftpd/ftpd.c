#ifndef lint
static	char sccsid[] = "@(#)ftpd.c 1.1 86/09/25 SMI"; /* from UCB 4.29 02/10/84 */
#endif
	...
	char *argv[];
{
	...
	if (argc != 1)
		_exit(1);
	if (sscanf(argv[0], "%lx.%hd",
	    &his_addr.sin_addr.s_addr, &his_addr.sin_port) != 2)
		_exit(1);
	his_addr.sin_family = AF_INET;
	his_addr.sin_addr.s_addr = htonl(his_addr.sin_addr.s_addr);
	his_addr.sin_port = htons(his_addr.sin_port);
